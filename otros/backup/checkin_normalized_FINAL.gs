...
// SIEMPRE usar el dominio custom api.check-inn.mx — apunta a us-central1
// (la única región activa). Las URLs directas *.run.app quedan prohibidas:
// hardcodearlas rompe cuando se retira una región (ver incidente 2026-08-27
// donde el sync leía de una región vieja sin fixes).
...
var WA_TEMPLATES_HEADERS = ["ID","Nombre","Body","Asunto","Schedule_type","Schedule_time","Alojamientos","Enabled","Created_at","Updated_at","Updated_by","Schedule_event","Schedule_offset"];
...
function waTemplatesUpsert_(data) {
  ...
  if (id && last >= 2) {
    ...
    for (var i = 0; i < ids.length; i++) {
      if (String(ids[i][0]||"").trim() === id) {
        ...
        range.setValues([[
          id, nombre, body, asunto, scheduleType, scheduleTime, alojamientos, enabled, existing[8] || now, now, updatedBy, scheduleEvent, scheduleOffset
        ]]);
  ...
  newRange.setValues([[id, nombre, body, asunto, scheduleType, scheduleTime, alojamientos, enabled, now, now, updatedBy, scheduleEvent, scheduleOffset]]);
