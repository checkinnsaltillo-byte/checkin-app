...
const RESERVACIONES_HEADERS = [
  ...
  "Lodgify Id",
  "Recibir WhatsApp","Número adicional WhatsApp",
...
function doPost(e) {
  try {
    ...
    if (action === "submit_checkout") return jsonOutput_(saveCheckout_(data));
    ...
    if (action === "llaves_notas_set")  return jsonOutput_(llavesNotasSet_(data));
    if (action === "bookings_by_guest") return jsonOutput_(bookingsByGuest_(data));
    // ─── Bot IA WhatsApp ───
    if (action === "wa_chat_context_get")    return jsonOutput_(waChatContextGet_(data));
    if (action === "wa_chat_context_append") return jsonOutput_(waChatContextAppend_(data));
    if (action === "wa_chat_conversations")  return jsonOutput_(waChatConversationsList_(data));
    if (action === "wa_chat_set_control")    return jsonOutput_(waChatSetControl_(data));
    if (action === "wa_chat_set_draft")      return jsonOutput_(waChatSetDraft_(data));
    if (action === "wa_bot_alojamientos")    return jsonOutput_(waBotAlojamientosList_());
    if (action === "wa_bot_alojamientos_set") return jsonOutput_(waBotAlojamientosSet_(data));
    if (action === "wa_all_messages")        return jsonOutput_(waAllMessagesByPhone_(data));
    if (action === "wa_set_late_checkout")   return jsonOutput_(waSetLateCheckout_(data));
    ...
    if (action === "bn_update_rows_bulk")       return jsonOutput_(bnUpdateRowsBulk_(data));
    if (action === "bn_bancos_delete_row")      return jsonOutput_(bnBancosDeleteRow_(data));
    if (action === "bn_bancos_dedupe_massive")  return jsonOutput_(bnBancosDedupeMassive_(data));
    if (action === "llaves_list")               return jsonOutput_(llavesList_());
    if (action === "llaves_upsert")             return jsonOutput_(llavesUpsert_(data));
    ...
    // ─── Alojamientos (editar catálogo desde admin) ───
    if (action === "save_alojamiento")          return jsonOutput_(saveAlojamiento_(data));
    // ─── WhatsApp automation (config + log) ───
    if (action === "wa_config_get_batch")       return jsonOutput_(waConfigGetBatch_(data));
    if (action === "wa_config_set")             return jsonOutput_(waConfigSet_(data));
    if (action === "wa_log_add")                return jsonOutput_(waLogAdd_(data));
    if (action === "wa_log_get_batch")          return jsonOutput_(waLogGetBatch_(data));
    if (action === "wa_inbound_log")            return jsonOutput_(waLogAdd_({ booking_id: "", tipo: "inbound", origin: "guest", to: data.to, from: data.from, sid: data.sid, status: "received", body_preview: data.body }));
    if (action === "wa_url_guia_get")           return jsonOutput_(waUrlGuiaGet_(data));
    if (action === "wa_scheduled_add")          return jsonOutput_(waScheduledAdd_(data));
    ...
    if (action === "wa_templates_list")         return jsonOutput_(waTemplatesList_());
    if (action === "wa_templates_upsert")       return jsonOutput_(waTemplatesUpsert_(data));
    if (action === "wa_templates_delete")       return jsonOutput_(waTemplatesDelete_(data));
    // ─── Reportes Técnicos (MVP F1) ───
    if (action === "rt_list")                   return jsonOutput_(rtList_(data));
    if (action === "rt_upsert")                 return jsonOutput_(rtUpsert_(data));
    if (action === "rt_delete")                 return jsonOutput_(rtDelete_(data));
    if (action === "rt_upload_image")           return jsonOutput_(rtUploadImage_(data));
    ...
    // ─── Pagos manuales ───
    if (action === "save_pago_manual")             return jsonOutput_(savePagoManual_(data));
    if (action === "delete_pago_manual")           return jsonOutput_(deletePagoManual_(data));
    // ─── Reservas: phones extra ───
    if (action === "save_reserva_phone_extra")     return jsonOutput_(saveReservaPhoneExtra_(data));
    if (action === "list_solicitudes")             return jsonOutput_(listSolicitudes_(data));
    if (action === "save_solicitud")               return jsonOutput_(saveSolicitud_(data));
    if (action === "update_solicitud_estado")      return jsonOutput_(updateSolicitudEstado_(data));
    if (action === "delete_reserva_phone_extra")   return jsonOutput_(deleteReservaPhoneExtra_(data));
    ...
    if (action === "inquilinos_upload_file")       return jsonOutput_(inquilinosUploadFile_(data));
    if (action === "inquilinos_pagos_list")        return jsonOutput_(inquilinosPagosList_(data));
    if (action === "inquilinos_pagos_save")        return jsonOutput_(inquilinosPagoSave_(data));
    if (action === "inquilinos_pagos_delete")      return jsonOutput_(inquilinosPagoDelete_(data));
    if (action === "inquilinos_pagos_update_facturapi") return jsonOutput_(inquilinosPagoUpdateFacturapi_(data));
    if (action === "inquilinos_pagos_archive_folio")    return jsonOutput_(inquilinosPagoArchiveFolio_(data));
    if (action === "inventarios_productos_list")     return jsonOutput_(inventariosProductosList_());
    if (action === "inventarios_producto_save")      return jsonOutput_(inventariosProductoSave_(data));
    if (action === "inventarios_producto_delete")    return jsonOutput_(inventariosProductoDelete_(data));
    if (action === "inventarios_producto_upload")    return jsonOutput_(inventariosUploadImagen_(data));
    if (action === "inventarios_stock_list")         return jsonOutput_(inventariosStockList_(data));
    if (action === "inventarios_stock_save")         return jsonOutput_(inventariosStockSave_(data));
    if (action === "inventarios_stock_delete")       return jsonOutput_(inventariosStockDelete_(data));
    if (action === "inventarios_movimiento_save")    return jsonOutput_(inventariosMovimientoSave_(data));
    if (action === "inventarios_movimientos_list")   return jsonOutput_(inventariosMovimientosList_(data));
    if (action === "inventarios_ordenes_list")       return jsonOutput_(inventariosOrdenesList_());
    if (action === "inventarios_orden_save")         return jsonOutput_(inventariosOrdenSave_(data));
    if (action === "inventarios_orden_delete")       return jsonOutput_(inventariosOrdenDelete_(data));
...
function doGet(e) {
  try {
    ...
    if (action === "bot_is_admin_phone") return jsonOutput_(botIsAdminPhone_(e.parameter || {}));
    if (action === "list_pagos_manuales")   return jsonOutput_(listPagosManuales_(e.parameter || {}));
    if (action === "save_pago_manual")      return jsonOutput_(savePagoManual_(e.parameter || {}));
    if (action === "delete_pago_manual")    return jsonOutput_(deletePagoManual_(e.parameter || {}));
    if (action === "list_reserva_phones_extra")  return jsonOutput_(listReservaPhonesExtra_(e.parameter || {}));
    if (action === "list_solicitudes")           return jsonOutput_(listSolicitudes_(e.parameter || {}));
    if (action === "save_solicitud")             return jsonOutput_(saveSolicitud_(e.parameter || {}));
    if (action === "update_solicitud_estado")    return jsonOutput_(updateSolicitudEstado_(e.parameter || {}));
    if (action === "save_reserva_phone_extra")   return jsonOutput_(saveReservaPhoneExtra_(e.parameter || {}));
    if (action === "delete_reserva_phone_extra") return jsonOutput_(deleteReservaPhoneExtra_(e.parameter || {}));
    ...
    if (action === "llaves_notas_list") return jsonOutput_(llavesNotasList_());
    if (action === "llaves_notas_set")  return jsonOutput_(llavesNotasSet_(e.parameter || {}));
    if (action === "bookings_by_guest") return jsonOutput_(bookingsByGuest_(e.parameter || {}));
    if (action === "wa_chat_context_get")    return jsonOutput_(waChatContextGet_(e.parameter || {}));
    if (action === "wa_chat_conversations")  return jsonOutput_(waChatConversationsList_(e.parameter || {}));
    if (action === "wa_bot_alojamientos")    return jsonOutput_(waBotAlojamientosList_());
    if (action === "wa_bot_alojamientos_set") return jsonOutput_(waBotAlojamientosSet_(e.parameter || {}));
    if (action === "wa_all_messages")        return jsonOutput_(waAllMessagesByPhone_(e.parameter || {}));
    if (action === "bot_prompts_list")       return jsonOutput_(botPromptsList_());
...
function updateReservacionWithFormData_(sheet, headers, row, recordId, data, cel, idVehiculo, now, ingresoDate) {
  ...
  waRegistroSyncRecipients_(sheet, headers, row, data);
  return { row_number: row, record_id: recordId, merged_with_lodgify_row: true };
}

/** Si el formulario de check-in incluye un Número adicional WhatsApp,
 *  hace upsert en WA_Config del booking (usando Lodgify Id si existe,
 *  o el ID de reservación si no) para que el modal WhatsApp del admin
 *  muestre ese teléfono automáticamente en "Números adicionales". */
function waRegistroSyncRecipients_(sheet, headers, row, data) {
  try {
    const numAd = safe_(data && data.numero_adicional_wa).trim();
    if (!numAd) return;
    const idxLod = headers.indexOf("Lodgify Id");
    const idxId  = headers.indexOf("ID");
    const lodgifyId = idxLod >= 0 ? String(sheet.getRange(row, idxLod + 1).getValue() || "").trim() : "";
    const resvId   = idxId  >= 0 ? String(sheet.getRange(row, idxId  + 1).getValue() || "").trim() : "";
    const bookingId = lodgifyId || resvId;
    if (!bookingId) return;
    const waNorm = _waNormalizeToWhatsapp_(numAd);
    if (!waNorm) return;
    waConfigSet_({ booking_id: bookingId, recipients: [waNorm], updated_by: "registro" });
  } catch(e) {
    Logger.log("[waRegistroSyncRecipients_] " + e);
  }
}

/** Convierte "'+52 8112345678" o similares a "whatsapp:+5218112345678".
 *  Reglas MX: si el número tiene 10 dígitos con lada 52, inserta el "1" que
 *  Twilio exige para móviles MX. Devuelve "" si no hay dígitos. */
function _waNormalizeToWhatsapp_(raw) {
  if (!raw) return "";
  var s = String(raw).replace(/[^0-9+]/g, "");
  // Quitar apóstrofe/prefijos raros: nos quedamos con dígitos y opcional '+' inicial
  var hasPlus = s.charAt(0) === "+";
  var digits = s.replace(/\D/g, "");
  if (!digits) return "";
  // Casos comunes:
  // - "5218XXXXXXXXXX" (13 dígitos, ya en formato Twilio MX) → +5218XXX...
  // - "528XXXXXXXXXX"  (12 dígitos MX sin el "1") → insertar "1" → +5218XXX...
  // - "8XXXXXXXXXX"    (10 dígitos MX sin lada) → +5218XXX...
  if (digits.length === 10) return "whatsapp:+521" + digits;
  if (digits.length === 12 && digits.slice(0, 2) === "52") return "whatsapp:+521" + digits.slice(2);
  if (digits.length === 13 && digits.slice(0, 3) === "521") return "whatsapp:+" + digits;
  // Fallback: si ya venía con "+", respetamos el número tal cual (para números
  // no-MX o con lada ya correcta).
  return hasPlus ? "whatsapp:+" + digits : "whatsapp:+" + digits;
}
...
/**
 * Rellena la columna "url_guia" en la hoja "alojamientos" con el link
 * público de la guía de bienvenida por HouseId. La crea si no existe.
 * Ejecutar UNA SOLA VEZ desde el editor de Apps Script:
 *   Seleccionar función → fillGuiaUrls → Ejecutar.
 *
 * Los alojamientos sin HouseId quedan con la celda vacía.
 */
function fillGuiaUrls() { return fillGuiaUrls_(); }
function fillGuiaUrls_() {
  const BASE = 'https://www.check-inn.mx/public/guia/?id=';
  const HEADER = 'url_guia';
  const ss = getSpreadsheet_();
  const sh = ss.getSheetByName(ALOJAMIENTOS_SHEET);
  if (!sh) return { ok: false, error: 'Hoja "alojamientos" no existe.' };
  const lastRow = sh.getLastRow();
  const lastCol = sh.getLastColumn();
  if (lastRow < 2) return { ok: false, error: 'Sin filas de datos.' };
  var headers = sh.getRange(1, 1, 1, lastCol).getValues()[0].map(function (v) { return String(v || '').trim(); });
  var idxHouse = headers.indexOf('HouseId') + 1;
  if (!idxHouse) return { ok: false, error: 'Columna HouseId no existe.' };
  var idxGuia = headers.indexOf(HEADER) + 1;
  if (!idxGuia) {
    idxGuia = lastCol + 1;
    sh.getRange(1, idxGuia).setValue(HEADER).setFontWeight('bold').setBackground('#dbeafe');
  }
  var ids = sh.getRange(2, idxHouse, lastRow - 1, 1).getDisplayValues();
  var out = [];
  var filled = 0;
  for (var i = 0; i < ids.length; i++) {
    var id = String(ids[i][0]).trim();
    if (id) { out.push([BASE + encodeURIComponent(id)]); filled++; }
    else out.push(['']);
  }
  sh.getRange(2, idxGuia, out.length, 1).setValues(out);
  return { ok: true, column: HEADER, filled: filled, totalRows: out.length };
}
...
function updateIncidencia_(data) {
  try {
    ...
    // Timestamp de última edición: auto-crear columna Updated_at si falta.
    var upCol = headers.indexOf('Updated_at') + 1;
    if (!upCol) {
      upCol = lastCol + 1;
      sh.getRange(1, upCol).setValue('Updated_at').setFontWeight('bold').setBackground('#fee2e2');
    }
    sh.getRange(rowIdx, upCol).setValue(Utilities.formatDate(new Date(), 'America/Monterrey', 'yyyy-MM-dd HH:mm:ss'));
...
function updateObjeto_(data) {
  try {
    ...
    if (!upCol) {
      upCol = lastCol + 1;
      sh.getRange(1, upCol).setValue('Updated_at').setFontWeight('bold').setBackground('#d1fae5');
    }
    sh.getRange(rowIdx, upCol).setValue(Utilities.formatDate(new Date(), 'America/Monterrey', 'yyyy-MM-dd HH:mm:ss'));
    return { ok: true, id: id, row: rowIdx, updated: updates };
  } catch (err) {
...
function rhListSimple_(sheetName) {
  try {
    ...
    if (!sh) return { ok: true, rows: [], headers: [] };
    var rows = rhRowsToObjects_(sh).filter(function (o) { return String(o['ID'] || '').trim(); });
    // Más nuevos primero (por Timestamp)
    rows.sort(function (a, b) { return String(b['Timestamp'] || '').localeCompare(String(a['Timestamp'] || '')); });
    // Headers del sheet — respeta el orden de columnas para render tabular
    var lastCol = Math.max(sh.getLastColumn(), 1);
    var headers = sh.getRange(1, 1, 1, lastCol).getValues()[0].map(function (v) { return String(v || '').trim(); }).filter(Boolean);
...
/**
 * Verifica si un teléfono (10 dígitos) pertenece a un admin.
 * Admin = fila en sys_users con Puesto="Administración" cuya columna
 * "cel" contenga ese número (los valores de esa columna son texto tipo
 * "'+528115569120'"; normalizamos a los últimos 10 dígitos para matchear).
 * Payload: { phone10: "8115569120" }.
 */
function botIsAdminPhone_(data) {
  try {
    var phone10 = String((data && (data.phone10 || data.phone)) || '').replace(/\D/g, '').slice(-10);
    if (!phone10) return { ok: false, isAdmin: false, error: 'phone10 requerido' };
    var sh = getSpreadsheet_().getSheetByName('sys_users');
    if (!sh) return { ok: false, isAdmin: false, error: 'Hoja sys_users no encontrada' };
    var values = sh.getDataRange().getDisplayValues();
    if (values.length < 2) return { ok: true, isAdmin: false };
    var headers = values[0].map(function (h) { return String(h || '').trim(); });
    var colPuesto = -1, colCel = -1, colNombre = -1;
    headers.forEach(function (h, idx) {
      var hl = h.toLowerCase();
      if (hl === 'puesto') colPuesto = idx;
      else if (hl === 'cel' || hl === 'celular' || hl === 'whatsapp') colCel = idx;
      else if (hl === 'nombre') colNombre = idx;
    });
    if (colPuesto < 0 || colCel < 0) {
      return { ok: false, isAdmin: false, error: 'Faltan columnas Puesto/cel en sys_users' };
    }
    for (var i = 1; i < values.length; i++) {
      var puesto = String(values[i][colPuesto] || '').trim().toLowerCase();
      if (puesto !== 'administración' && puesto !== 'administracion') continue;
      var cel10 = String(values[i][colCel] || '').replace(/\D/g, '').slice(-10);
      if (cel10 && cel10 === phone10) {
        var nombre = colNombre >= 0 ? String(values[i][colNombre] || '').trim() : '';
        return { ok: true, isAdmin: true, nombre: nombre };
      }
    }
    return { ok: true, isAdmin: false };
  } catch (err) {
    return { ok: false, isAdmin: false, error: String(err && err.message || err) };
  }
}

// ═══════════════════════════════════════════════════════════════════════════
// ║ PAGOS MANUALES — pagos fuera de Stripe/Lodgify (efectivo, transferencia) ║
// ║ Hoja: Pagos_manuales                                                     ║
// ║ Columnas: ID · Timestamp · ReservaId · Monto · Metodo · Fecha ·          ║
// ║           Referencia · Notas · RegistradoPor                             ║
// ═══════════════════════════════════════════════════════════════════════════
var PAGOS_MANUALES_SHEET   = 'Pagos_manuales';
var PAGOS_MANUALES_HEADERS = ['ID','Timestamp','ReservaId','Monto','Metodo','Fecha','Referencia','Notas','RegistradoPor','EvidenciasJSON'];

function ensurePagosManualesSheet_() {
  var ss = getSpreadsheet_();
  var sh = ss.getSheetByName(PAGOS_MANUALES_SHEET);
  if (!sh) {
    sh = ss.insertSheet(PAGOS_MANUALES_SHEET);
    sh.getRange(1, 1, 1, PAGOS_MANUALES_HEADERS.length).setValues([PAGOS_MANUALES_HEADERS]);
    sh.getRange(1, 1, 1, PAGOS_MANUALES_HEADERS.length).setFontWeight('bold').setBackground('#f1f5f9');
    sh.setFrozenRows(1);
  } else {
    // Migración: agregar columnas nuevas si faltan (ej. EvidenciasJSON).
    var lastCol = Math.max(sh.getLastColumn(), 1);
    var current = sh.getRange(1, 1, 1, lastCol).getValues()[0].map(function (h) { return String(h || '').trim(); });
    var missing = PAGOS_MANUALES_HEADERS.filter(function (h) { return current.indexOf(h) < 0; });
    if (missing.length) {
      sh.getRange(1, current.length + 1, 1, missing.length).setValues([missing]);
      sh.getRange(1, current.length + 1, 1, missing.length).setFontWeight('bold').setBackground('#f1f5f9');
    }
  }
  return sh;
}

function listPagosManuales_(data) {
  try {
    var sh = ensurePagosManualesSheet_();
    var last = sh.getLastRow();
    if (last < 2) return { ok: true, rows: [] };
    var headers = sh.getRange(1, 1, 1, PAGOS_MANUALES_HEADERS.length).getValues()[0];
    var values = sh.getRange(2, 1, last - 1, PAGOS_MANUALES_HEADERS.length).getValues();
    var reservaId = String((data && data.reservaId) || '').trim();
    var rows = values.map(function (r) {
      var o = {};
      headers.forEach(function (h, i) { o[h] = r[i]; });
      return o;
    });
    if (reservaId) rows = rows.filter(function (r) { return String(r.ReservaId || '').trim() === reservaId; });
...
function savePagoManual_(data) {
  try {
    var payload = data && data.payload
      ? (typeof data.payload === 'string' ? JSON.parse(data.payload) : data.payload)
      : data;
    var reservaId = String((payload && payload.ReservaId) || '').trim();
    var monto = Number((payload && payload.Monto) || 0);
    if (!reservaId) return { ok: false, error: 'ReservaId requerido' };
    if (!(monto > 0)) return { ok: false, error: 'Monto debe ser > 0' };
    var sh = ensurePagosManualesSheet_();
    var now = new Date();
    var id = 'PM-' + Utilities.formatDate(now, Session.getScriptTimeZone(), 'yyyyMMdd-HHmmss') + '-' + Math.floor(Math.random() * 9000 + 1000);
    var fecha = String((payload && payload.Fecha) || '').trim() || Utilities.formatDate(now, Session.getScriptTimeZone(), 'yyyy-MM-dd');
    // Evidencias: array [{tipo:'text'|'image', body:'', media_url:'', timestamp:''}]
    var evid = (payload && payload.Evidencias) || (payload && payload.EvidenciasJSON) || [];
    var evidStr = '';
    try {
      if (typeof evid === 'string') { JSON.parse(evid); evidStr = evid; }
      else if (Array.isArray(evid)) { evidStr = JSON.stringify(evid); }
    } catch (_) { evidStr = ''; }
    var metodo = String((payload && payload.Metodo) || '').trim();
    var referencia = String((payload && payload.Referencia) || '').trim();
    var notas = String((payload && payload.Notas) || '').trim();
    var registradoPor = String((payload && payload.RegistradoPor) || '').trim();
    // Dedup: si ya existe una fila con mismo (ReservaId, Monto, Fecha, Referencia)
    // — con Referencia opcional — devuelve el ID existente en vez de crear duplicado.
    // Regla: si Referencia trae valor, debe coincidir; si viene vacía, se dedupea
    // solo por (ReservaId, Monto, Fecha, Metodo).
    var last = sh.getLastRow();
    if (last >= 2) {
      var existing = sh.getRange(2, 1, last - 1, 10).getValues();
      for (var i = 0; i < existing.length; i++) {
        var e = existing[i];
        var eId = String(e[0] || '');
        var eReserva = String(e[2] || '').trim();
        var eMonto = Number(e[3]) || 0;
        var eMetodo = String(e[4] || '').trim();
        var eFecha = String(e[5] || '').trim().slice(0, 10);
        var eRef = String(e[6] || '').trim();
        if (eReserva !== reservaId) continue;
        if (Math.abs(eMonto - monto) > 0.01) continue;
        var fechaKey = String(fecha).slice(0, 10);
        if (eFecha && fechaKey && eFecha !== fechaKey) continue;
        if (referencia) {
          if (eRef === referencia) return { ok: true, id: eId, deduped: true };
        } else {
          if (!eRef && eMetodo === metodo) return { ok: true, id: eId, deduped: true };
        }
      }
    }
    var row = [
...
function deletePagoManual_(data) {
  try {
    var payload = data && data.payload
      ? (typeof data.payload === 'string' ? JSON.parse(data.payload) : data.payload)
      : data;
    var id = String((payload && payload.id) || (data && data.id) || '').trim();
...
// ═══════════════════════════════════════════════════════════════════════════
// ║ RESERVAS · PHONES EXTRA — asocia n. celulares adicionales a una reserva ║
// ║ Hoja: Reservas_phones_extra                                              ║
// ║ Columnas: ID · Timestamp · ReservaId · Phone · AddedBy                   ║
// ║ Usado por Chats bot para vincular manualmente una reserva Lodgify a     ║
// ║ un número de WhatsApp distinto al que Lodgify tiene registrado.         ║
// ═══════════════════════════════════════════════════════════════════════════
var RESERVAS_PHONES_EXTRA_SHEET   = 'Reservas_phones_extra';
var RESERVAS_PHONES_EXTRA_HEADERS = ['ID','Timestamp','ReservaId','Phone','AddedBy'];

function ensureReservasPhonesExtraSheet_() {
  var ss = getSpreadsheet_();
  var sh = ss.getSheetByName(RESERVAS_PHONES_EXTRA_SHEET);
  if (!sh) {
    sh = ss.insertSheet(RESERVAS_PHONES_EXTRA_SHEET);
    sh.getRange(1, 1, 1, RESERVAS_PHONES_EXTRA_HEADERS.length).setValues([RESERVAS_PHONES_EXTRA_HEADERS]);
    sh.getRange(1, 1, 1, RESERVAS_PHONES_EXTRA_HEADERS.length).setFontWeight('bold').setBackground('#f1f5f9');
    sh.setFrozenRows(1);
  }
  return sh;
}
function _rpeReadAll_() {
  var sh = ensureReservasPhonesExtraSheet_();
  var last = sh.getLastRow();
  if (last < 2) return [];
  var values = sh.getRange(2, 1, last - 1, RESERVAS_PHONES_EXTRA_HEADERS.length).getValues();
  return values.map(function (r) {
    var o = {};
    RESERVAS_PHONES_EXTRA_HEADERS.forEach(function (h, i) { o[h] = r[i]; });
    return o;
  });
}
function listReservaPhonesExtra_(data) {
  try {
    var phone10 = String((data && data.phone) || '').replace(/\D/g,'').slice(-10);
    var reservaId = String((data && data.reservaId) || '').trim();
    var rows = _rpeReadAll_();
    if (phone10) rows = rows.filter(function(r){ return String(r.Phone||'').replace(/\D/g,'').slice(-10) === phone10; });
    if (reservaId) rows = rows.filter(function(r){ return String(r.ReservaId||'').trim() === reservaId; });
    return { ok: true, rows: rows };
  } catch (err) { return { ok: false, error: String(err && err.message || err) }; }
}
function saveReservaPhoneExtra_(data) {
  try {
    var payload = data && data.payload
      ? (typeof data.payload === 'string' ? JSON.parse(data.payload) : data.payload)
      : data;
    var reservaId = String((payload && payload.ReservaId) || '').trim();
    var phone10 = String((payload && payload.Phone) || '').replace(/\D/g,'').slice(-10);
    if (!reservaId) return { ok: false, error: 'ReservaId requerido' };
    if (!phone10 || phone10.length < 10) return { ok: false, error: 'Phone inválido (10+ dígitos)' };
    // Evita duplicados
    var existing = _rpeReadAll_().find(function (r) {
      return String(r.ReservaId||'').trim() === reservaId
          && String(r.Phone||'').replace(/\D/g,'').slice(-10) === phone10;
    });
    if (existing) return { ok: true, id: existing.ID, duplicate: true };
    var sh = ensureReservasPhonesExtraSheet_();
    var now = new Date();
    var id = 'RPE-' + Utilities.formatDate(now, Session.getScriptTimeZone(), 'yyyyMMdd-HHmmss') + '-' + Math.floor(Math.random()*9000+1000);
    sh.appendRow([
      id,
      now.toISOString(),
      reservaId,
      phone10,
      String((payload && payload.AddedBy) || '').trim(),
    ]);
    return { ok: true, id: id };
  } catch (err) { return { ok: false, error: String(err && err.message || err) }; }
}
function deleteReservaPhoneExtra_(data) {
  try {
    var payload = data && data.payload
      ? (typeof data.payload === 'string' ? JSON.parse(data.payload) : data.payload)
      : data;
    ...
    var ids = sh.getRange(2, 1, last - 1, 1).getDisplayValues();
    for (var i = 0; i < ids.length; i++) {
      if (String(ids[i][0]).trim() === id) { sh.deleteRow(i + 2); return { ok: true, id: id }; }
    }
    return { ok: false, error: 'ID no encontrado' };
  } catch (err) { return { ok: false, error: String(err && err.message || err) }; }
}
...
// ═══════════════════════════════════════════════════════════════════════════
// ║ Hoja: Solicitudes_Pendientes                                             ║
// ║ Columnas: ID · Timestamp · Phone · Tipo · ReservaId · Resumen · Estado  ║
// ║           · AtendidoPor · AtendidoAt · Notas                             ║
// ║ Estado: 'pendiente' (default) → 'atendido' | 'cancelado'                ║
// ║ Tipos: 'ticket_autofacturacion' (extensible)                             ║
// ║ Genera cada vez que el bot ejecuta solicitar_ticket_admin (u otros).    ║
// ═══════════════════════════════════════════════════════════════════════════
var SOLICITUDES_SHEET = 'Solicitudes_Pendientes';
var SOLICITUDES_HEADERS = ['ID','Timestamp','Phone','Tipo','ReservaId','Resumen','Estado','AtendidoPor','AtendidoAt','Notas','ProgramadaAt'];
function ensureSolicitudesSheet_() {
  var ss = getSpreadsheet_();
  var sh = ss.getSheetByName(SOLICITUDES_SHEET);
  if (!sh) {
    sh = ss.insertSheet(SOLICITUDES_SHEET);
    sh.getRange(1, 1, 1, SOLICITUDES_HEADERS.length).setValues([SOLICITUDES_HEADERS]);
    sh.getRange(1, 1, 1, SOLICITUDES_HEADERS.length).setFontWeight('bold').setBackground('#fef3c7');
    sh.setFrozenRows(1);
  }
  return sh;
}
function _solRead_() {
  var sh = ensureSolicitudesSheet_();
  var last = sh.getLastRow();
  if (last < 2) return [];
  var values = sh.getRange(2, 1, last - 1, SOLICITUDES_HEADERS.length).getValues();
  return values.map(function(r){ var o={}; SOLICITUDES_HEADERS.forEach(function(h,i){o[h]=r[i];}); return o; });
}
function listSolicitudes_(data) {
  try {
    var phone10 = String((data && data.phone) || '').replace(/\D/g,'').slice(-10);
    var estado = String((data && data.estado) || '').trim().toLowerCase();
    var rows = _solRead_();
    if (phone10) rows = rows.filter(function(r){ return String(r.Phone||'').replace(/\D/g,'').slice(-10) === phone10; });
    if (estado) rows = rows.filter(function(r){ return String(r.Estado||'').toLowerCase() === estado; });
    // Ordenar por Timestamp desc (más reciente arriba)
    rows.sort(function(a,b){ return String(b.Timestamp||'').localeCompare(String(a.Timestamp||'')); });
    return { ok:true, rows: rows };
  } catch (err) { return { ok:false, error: String(err && err.message || err) }; }
}
function saveSolicitud_(data) {
  try {
    var payload = data && data.payload
      ? (typeof data.payload === 'string' ? JSON.parse(data.payload) : data.payload)
      : data;
    ...
    if (!phone10 || phone10.length < 10) return { ok:false, error:'Phone inválido' };
    if (!tipo) return { ok:false, error:'Tipo requerido' };
    var sh = ensureSolicitudesSheet_();
    var now = new Date();
    var id = 'SOL-' + Utilities.formatDate(now, Session.getScriptTimeZone(), 'yyyyMMdd-HHmmss') + '-' + Math.floor(Math.random()*9000+1000);
    sh.appendRow([
      id,
      now.toISOString(),
      phone10,
      tipo,
      String((payload && payload.ReservaId) || '').trim(),
      String((payload && payload.Resumen) || '').trim(),
      'pendiente',
      '',
      '',
      String((payload && payload.Notas) || '').trim(),
      '', // ProgramadaAt — se llena cuando el admin programa
    ]);
    return { ok:true, id:id };
  } catch (err) { return { ok:false, error: String(err && err.message || err) }; }
}
function updateSolicitudEstado_(data) {
  try {
    var payload = data && data.payload
      ? (typeof data.payload === 'string' ? JSON.parse(data.payload) : data.payload)
      : data;
    var id = String((payload && payload.id) || (data && data.id) || '').trim();
    var estado = String((payload && payload.estado) || '').trim().toLowerCase();
    if (!id) return { ok:false, error:'id requerido' };
    if (!['pendiente','atendido','cancelado','aprobado','rechazado','programado'].includes(estado)) return { ok:false, error:'estado inválido' };
    var sh = ensureSolicitudesSheet_();
    var last = sh.getLastRow();
    if (last < 2) return { ok:false, error:'Hoja vacía' };
    var vals = sh.getRange(2, 1, last - 1, SOLICITUDES_HEADERS.length).getValues();
    for (var i=0;i<vals.length;i++) {
      if (String(vals[i][0]).trim() === id) {
        var row = i + 2;
        sh.getRange(row, 7).setValue(estado);
        sh.getRange(row, 8).setValue(String((payload && payload.AtendidoPor) || ''));
        sh.getRange(row, 9).setValue(new Date().toISOString());
        if (payload && payload.Notas) sh.getRange(row, 10).setValue(String(payload.Notas));
        // ProgramadaAt (columna 11) — solo si viene en el payload.
        if (payload && payload.ProgramadaAt) sh.getRange(row, 11).setValue(String(payload.ProgramadaAt));
        return { ok:true, id:id };
      }
    }
...
function updateFacturapiFolioStrict_(data) {
  ...
  if (String(externalId).indexOf('INQPAGO-') === 0 || String(recordId).indexOf('INQPAGO-') === 0) {
    return inquilinosPagoUpdateFacturapi_(data);
  }
...
function saveFacturapiPdf_(data) {
  ...
  // Ruta al handler de Inquilinos_Pagos cuando externalId trae prefix INQPAGO-.
  if (String(externalId).indexOf('INQPAGO-') === 0 || String(recordId).indexOf('INQPAGO-') === 0) {
    return inquilinosPagoSaveFacturapiPdf_(data);
  }
  const sheet = getSheet_(RESERVACIONES_SHEET);
  const headers = getHeaders_(sheet);
  let row = null;
...
function getNextFacturapiFolio_() {
  ensureNormalizedSheets_();
  let maxFolio = 0;
  const scanCol = (sheet, headerName) => {
    if (!sheet) return;
    const headers = getHeaders_(sheet);
    const col = headers.indexOf(headerName);
    if (col < 0 || sheet.getLastRow() <= 1) return;
    const values = sheet.getRange(2, col + 1, sheet.getLastRow() - 1, 1).getDisplayValues();
    values.forEach(r => {
      // Puede haber múltiples folios separados por coma (Folio_antiguo).
      String(r[0] || '').split(/[,;\s]+/).forEach(tok => {
        const v = tok.replace(/[^0-9]/g, '');
        const n = Number(v || 0);
        if (n > maxFolio) maxFolio = n;
      });
    });
  };
  // 1) Reservaciones (Folio facturapi + antiguo).
  const shRes = getSheet_(RESERVACIONES_SHEET);
  scanCol(shRes, 'Folio facturapi');
  scanCol(shRes, 'Folio facturapi antiguo');
  // 2) Inquilinos_Pagos (Folio_facturapi + Folio_facturapi_antiguo) — evita
  // reuso de folios ya emitidos para inquilinos (o archivados por Re-emitir).
  try {
    const shInq = ensureInquilinosPagosSheet_();
    scanCol(shInq, 'Folio_facturapi');
    scanCol(shInq, 'Folio_facturapi_antiguo');
  } catch (_) {}
...
// ═══ Check-out: guarda cada envío del formulario "Registrar salida" ═══════
// Origen: /public/registro/ (portal huésped y guías de bienvenida vía iframe).
function saveCheckout_(data) {
  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const SHEET = "check_out";
    let sh = ss.getSheetByName(SHEET);
    if (!sh) {
      sh = ss.insertSheet(SHEET);
      sh.appendRow(["Timestamp", "Propiedad", "# Departamento", "Fecha y hora", "Comentarios"]);
      sh.getRange(1, 1, 1, 5).setFontWeight("bold").setBackground("#f1f5f9");
      sh.setFrozenRows(1);
    }
    sh.appendRow([
      new Date(),
      String(data.propiedad || ""),
      String(data.depto || ""),
    ...
    Logger.log("[saveCheckout_] " + err);
    return { ok: false, error: String(err) };
  }
}

// ═══ INQUILINOS ═════════════════════════════════════════════════════════════
// Módulo de gestión de rentas a largo plazo (distinto de huéspedes de estancia).
// Persiste dos hojas + carpeta Drive dedicada:
//   · Inquilinos          — perfil + contrato + identificación
//   · Inquilinos_Pagos    — pagos mensuales por inquilino (renta)
//   · Drive/Inquilinos/{ID}/  — contratos y fotos de ID

const INQUILINOS_SHEET = "Inquilinos";
const INQUILINOS_PAGOS_SHEET = "Inquilinos_Pagos";
const INQUILINOS_HEADERS = [
  "ID","Nombre","Propiedad","Departamento","Clasificacion","Tipo_propiedad","Contrato_existe","Fecha_inicio","Duracion_meses","Fecha_fin","Estado_contrato",
  "Requiere_factura","Razon_social","RFC","Regimen_fiscal","CP","Correo_factura","Renta_incluye_impuestos","CIF_files_json",
  "Renta_mensual","Metodo_pago","Dia_pago",
  "Correo","Whatsapp","Contacto_emerg_nombre","Contacto_emerg_cel",
  "Servicios_estado","Servicios_incluidos","Notas","Servicios_json","Amueblado","Muebles_json",
  "Deposito","Aval_nombre","Aval_whatsapp","Aval_identificacion_tipo","Aval_identificacion_otro_desc","Aval_identificacion_files_json",
  "Contrato_files_json","Identificacion_tipo","Identificacion_otro_desc","Identificacion_files_json",
  "Comentarios",
  "created_at","updated_at"
];
const INQUILINOS_PAGOS_HEADERS = [
  "ID","Inquilino_ID","Mes","Monto_pagado","Metodo_pago","Fecha_pago",
  "Comprobante_url","Notas","created_at","Comprobante_files_json",
  "Folio_facturapi","Folio_facturapi_antiguo","Organizacion_facturapi","Ticket_facturapi_url","Ticket_facturapi_id_archivo","Ticket_facturapi_nombre_archivo","Ticket_facturapi_carpeta_url","Ticket_facturapi_carpeta_ruta","Estatus_facturapi","Fecha_emision"
];

// ═══════════════════════════════════════════════════════════════════════
// ═══ MÓDULO INVENTARIOS ═══════════════════════════════════════════════
// 4 hojas: Productos (catálogo), Stock (por ubicación), Movimientos
// (historial), Ordenes (compras). Todas con headers auto-heal.
// ═══════════════════════════════════════════════════════════════════════
const INVENTARIOS_PRODUCTOS_SHEET   = 'Inventarios_Productos';
const INVENTARIOS_STOCK_SHEET       = 'Inventarios_Stock';
const INVENTARIOS_MOVIMIENTOS_SHEET = 'Inventarios_Movimientos';
const INVENTARIOS_ORDENES_SHEET     = 'Inventarios_Ordenes';

const INVENTARIOS_PRODUCTOS_HEADERS = [
  'ID','Codigo','Producto','Naturaleza','Clase','Driver_consumo','Control',
  'Cantidad_default','Unidad','Precio_unitario','Precio_total',
  'Tiempo_entrega_dias','Medio_compra','Lugar_compra','Marketplace_url',
  'Stock_minimo','Imagen_url','Imagen_id','Notas','created_at','updated_at'
];
const INVENTARIOS_STOCK_HEADERS = [
  'ID','Producto_ID','Propiedad','Departamento','Ubicacion_extra','Estado',
  'Cantidad','Unidad','Stock_minimo_local','Ultima_reposicion','Ultimo_usuario',
  'Notas','created_at','updated_at'
];
const INVENTARIOS_MOVIMIENTOS_HEADERS = [
  'ID','Stock_ID','Producto_ID','Fecha','Usuario','Tipo','Cantidad_delta',
  'Cantidad_despues','Notas','created_at'
];
const INVENTARIOS_ORDENES_HEADERS = [
  'ID','Fecha','Usuario','Tipo_reposicion','Subtipo','Estado','Items_json',
  'Total','Proveedor','Notas','created_at','updated_at'
];

function _invEnsureSheet_(name, headers) {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  let sh = ss.getSheetByName(name);
  if (!sh) sh = ss.insertSheet(name);
  inquilinosEnsureHeaders_(sh, headers); // reusa el auto-heal existente
  return sh;
}

function _invRowsToObjects_(sh, headers) {
  ...
  return vals.map(r => {
    ...
    // Deserializa _json a _js field paralelo
    ['Items_json'].forEach(k => {
      if (o[k]) { try { o[k.replace('_json','')] = JSON.parse(o[k]); } catch(_){ o[k.replace('_json','')] = []; } }
    });
    return o;
  });
}

function _invNewId_(prefix) {
  return prefix + '-' + Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyyMMddHHmmss') + '-' + Math.floor(Math.random() * 10000);
}

// ── PRODUCTOS ──
function inventariosProductosList_() {
  try {
    const sh = _invEnsureSheet_(INVENTARIOS_PRODUCTOS_SHEET, INVENTARIOS_PRODUCTOS_HEADERS);
    return { ok: true, rows: _invRowsToObjects_(sh, INVENTARIOS_PRODUCTOS_HEADERS) };
  } catch (err) { return { ok: false, error: String(err) }; }
}
function inventariosProductoSave_(data) {
  try {
    data = data || {};
    const sh = _invEnsureSheet_(INVENTARIOS_PRODUCTOS_SHEET, INVENTARIOS_PRODUCTOS_HEADERS);
    let id = String(data.ID || '').trim();
    const isNew = !id;
    if (isNew) id = _invNewId_('PRD');
    const now = new Date().toISOString();
    // Localiza fila existente y hace save parcial (preserva no enviados).
    let foundRow = 0; const existing = {};
    if (!isNew) {
      const last = sh.getLastRow();
      if (last > 1) {
        const ids = sh.getRange(2, 1, last - 1, 1).getValues();
        for (let i = 0; i < ids.length; i++) if (String(ids[i][0] || '').trim() === id) { foundRow = i + 2; break; }
      }
      if (foundRow) {
        const cur = sh.getRange(foundRow, 1, 1, INVENTARIOS_PRODUCTOS_HEADERS.length).getValues()[0];
        INVENTARIOS_PRODUCTOS_HEADERS.forEach((h, ix) => { existing[h] = cur[ix]; });
      }
    }
    // Auto-calcula Precio_total si viene Precio_unitario + Cantidad
    let pTotal = data.Precio_total;
    if ((pTotal == null || pTotal === '') && data.Precio_unitario != null && data.Cantidad_default != null) {
      const pu = parseFloat(String(data.Precio_unitario).replace(/[^0-9.-]/g,''));
      const qt = parseFloat(String(data.Cantidad_default).replace(/[^0-9.-]/g,''));
      if (!isNaN(pu) && !isNaN(qt)) pTotal = (pu * qt).toFixed(2);
    }
    const row = INVENTARIOS_PRODUCTOS_HEADERS.map(h => {
      if (h === 'ID') return id;
      if (h === 'created_at') return isNew ? now : (existing.created_at || data.created_at || now);
      if (h === 'updated_at') return now;
      if (h === 'Precio_total' && pTotal != null) return pTotal;
      if (Object.prototype.hasOwnProperty.call(data, h)) return String(data[h] == null ? '' : data[h]);
      return existing[h] == null ? '' : existing[h];
    });
    if (isNew || !foundRow) sh.appendRow(row);
    else sh.getRange(foundRow, 1, 1, INVENTARIOS_PRODUCTOS_HEADERS.length).setValues([row]);
    return { ok: true, ID: id };
  } catch (err) { return { ok: false, error: String(err) }; }
}
function inventariosProductoDelete_(data) {
  try {
...
function inventariosDriveRoot_() {
  const rootId = getInventariosDriveRootId_();
  return DriveApp.getFolderById(rootId);
}
function getInventariosDriveRootId_() {
  // Reusa la misma carpeta root de inquilinos si existe; si no, crea "Inventarios".
  try {
    const parent = inquilinosDriveRoot_().getParents().next();
    const it = parent.getFoldersByName('Inventarios');
    if (it.hasNext()) return it.next().getId();
    return parent.createFolder('Inventarios').getId();
  } catch (_) {
    // Fallback: raíz de Drive
    const it = DriveApp.getRootFolder().getFoldersByName('Inventarios');
    if (it.hasNext()) return it.next().getId();
    return DriveApp.getRootFolder().createFolder('Inventarios').getId();
  }
}
function inventariosUploadImagen_(data) {
  try {
    data = data || {};
    const productoId = String(data.producto_id || '').trim() || 'sin_id';
    const fname = String(data.filename || 'imagen.jpg').trim();
    const mime = String(data.mime || 'image/jpeg');
    const b64 = String(data.data || '').replace(/^data:[^;]*;base64,/, '');
    if (!b64) return { ok: false, error: 'data vacío' };
    const root = inventariosDriveRoot_();
    const folder = getOrCreateFolder_(root, productoId);
    ...
    return {
      ok: true,
      file: {
        id: fileId, name: fname, mime: mime,
...
// ── STOCK ──
function inventariosStockList_(data) {
  try {
    const sh = _invEnsureSheet_(INVENTARIOS_STOCK_SHEET, INVENTARIOS_STOCK_HEADERS);
    let rows = _invRowsToObjects_(sh, INVENTARIOS_STOCK_HEADERS);
    const pid = String((data && data.producto_id) || '').trim();
    if (pid) rows = rows.filter(r => String(r.Producto_ID || '').trim() === pid);
    return { ok: true, rows };
  } catch (err) { return { ok: false, error: String(err) }; }
}
function inventariosStockSave_(data) {
  try {
    data = data || {};
    const sh = _invEnsureSheet_(INVENTARIOS_STOCK_SHEET, INVENTARIOS_STOCK_HEADERS);
    let id = String(data.ID || '').trim();
    const isNew = !id;
    if (isNew) id = _invNewId_('STK');
    const now = new Date().toISOString();
    let foundRow = 0; const existing = {};
    if (!isNew) {
      const last = sh.getLastRow();
      ...
      if (foundRow) {
        const cur = sh.getRange(foundRow, 1, 1, INVENTARIOS_STOCK_HEADERS.length).getValues()[0];
        INVENTARIOS_STOCK_HEADERS.forEach((h, ix) => { existing[h] = cur[ix]; });
      }
    }
    const row = INVENTARIOS_STOCK_HEADERS.map(h => {
      if (h === 'ID') return id;
      if (h === 'created_at') return isNew ? now : (existing.created_at || data.created_at || now);
      if (h === 'updated_at') return now;
      if (Object.prototype.hasOwnProperty.call(data, h)) return String(data[h] == null ? '' : data[h]);
    ...
    if (isNew || !foundRow) sh.appendRow(row);
    else sh.getRange(foundRow, 1, 1, INVENTARIOS_STOCK_HEADERS.length).setValues([row]);
    return { ok: true, ID: id };
  } catch (err) { return { ok: false, error: String(err) }; }
}
function inventariosStockDelete_(data) {
  try {
    const id = String((data && data.ID) || '').trim();
    if (!id) return { ok: false, error: 'ID requerido' };
    const sh = _invEnsureSheet_(INVENTARIOS_STOCK_SHEET, INVENTARIOS_STOCK_HEADERS);
...
// Movimiento de reposición: ajusta cantidad + registra en historial
function inventariosMovimientoSave_(data) {
  try {
    data = data || {};
    const stockId = String(data.Stock_ID || '').trim();
    if (!stockId) return { ok: false, error: 'Stock_ID requerido' };
    const shStock = _invEnsureSheet_(INVENTARIOS_STOCK_SHEET, INVENTARIOS_STOCK_HEADERS);
    const last = shStock.getLastRow();
    let foundRow = 0;
    if (last >= 2) {
      const ids = shStock.getRange(2, 1, last - 1, 1).getValues();
      for (let i = 0; i < ids.length; i++) if (String(ids[i][0] || '').trim() === stockId) { foundRow = i + 2; break; }
    }
    if (!foundRow) return { ok: false, error: 'Stock no encontrado' };
    const cur = shStock.getRange(foundRow, 1, 1, INVENTARIOS_STOCK_HEADERS.length).getValues()[0];
    const stockObj = {}; INVENTARIOS_STOCK_HEADERS.forEach((h, i) => { stockObj[h] = cur[i]; });
    const before = parseFloat(String(stockObj.Cantidad || '0').replace(/[^0-9.-]/g,'')) || 0;
    const delta = parseFloat(String(data.Cantidad_delta || '0').replace(/[^0-9.-]/g,'')) || 0;
    const after = before + delta;
    // Escribir nuevo saldo + ultimo usuario + ultima reposicion
    const cQty = INVENTARIOS_STOCK_HEADERS.indexOf('Cantidad') + 1;
    const cRep = INVENTARIOS_STOCK_HEADERS.indexOf('Ultima_reposicion') + 1;
    const cUsr = INVENTARIOS_STOCK_HEADERS.indexOf('Ultimo_usuario') + 1;
    const cUpd = INVENTARIOS_STOCK_HEADERS.indexOf('updated_at') + 1;
    const now = new Date().toISOString();
    shStock.getRange(foundRow, cQty).setValue(after);
    shStock.getRange(foundRow, cRep).setValue(now);
    shStock.getRange(foundRow, cUsr).setValue(String(data.Usuario || ''));
    shStock.getRange(foundRow, cUpd).setValue(now);
    // Registrar historial
    const shMov = _invEnsureSheet_(INVENTARIOS_MOVIMIENTOS_SHEET, INVENTARIOS_MOVIMIENTOS_HEADERS);
    const movId = _invNewId_('MOV');
    shMov.appendRow(INVENTARIOS_MOVIMIENTOS_HEADERS.map(h => {
      if (h === 'ID') return movId;
      if (h === 'Stock_ID') return stockId;
      if (h === 'Producto_ID') return String(stockObj.Producto_ID || '');
      if (h === 'Fecha') return data.Fecha || now;
      if (h === 'Usuario') return String(data.Usuario || '');
      if (h === 'Tipo') return String(data.Tipo || 'reposicion');
      if (h === 'Cantidad_delta') return delta;
      if (h === 'Cantidad_despues') return after;
      if (h === 'Notas') return String(data.Notas || '');
      if (h === 'created_at') return now;
      return '';
    }));
    return { ok: true, movimiento_id: movId, cantidad_antes: before, cantidad_despues: after };
  } catch (err) { return { ok: false, error: String(err) }; }
}
function inventariosMovimientosList_(data) {
  try {
    const sh = _invEnsureSheet_(INVENTARIOS_MOVIMIENTOS_SHEET, INVENTARIOS_MOVIMIENTOS_HEADERS);
    let rows = _invRowsToObjects_(sh, INVENTARIOS_MOVIMIENTOS_HEADERS);
    const sid = String((data && data.stock_id) || '').trim();
    const pid = String((data && data.producto_id) || '').trim();
    if (sid) rows = rows.filter(r => String(r.Stock_ID || '') === sid);
    else if (pid) rows = rows.filter(r => String(r.Producto_ID || '') === pid);
...
// ── ORDENES DE COMPRA ──
function inventariosOrdenesList_() {
  try {
    const sh = _invEnsureSheet_(INVENTARIOS_ORDENES_SHEET, INVENTARIOS_ORDENES_HEADERS);
    return { ok: true, rows: _invRowsToObjects_(sh, INVENTARIOS_ORDENES_HEADERS) };
  } catch (err) { return { ok: false, error: String(err) }; }
}
function inventariosOrdenSave_(data) {
  try {
    data = data || {};
    const sh = _invEnsureSheet_(INVENTARIOS_ORDENES_SHEET, INVENTARIOS_ORDENES_HEADERS);
    let id = String(data.ID || '').trim();
    const isNew = !id;
    if (isNew) id = _invNewId_('OC');
    const now = new Date().toISOString();
    let foundRow = 0; const existing = {};
    if (!isNew) {
      const last = sh.getLastRow();
      ...
      if (foundRow) {
        const cur = sh.getRange(foundRow, 1, 1, INVENTARIOS_ORDENES_HEADERS.length).getValues()[0];
        INVENTARIOS_ORDENES_HEADERS.forEach((h, ix) => { existing[h] = cur[ix]; });
      }
    }
    const itemsJson = Array.isArray(data.Items) ? JSON.stringify(data.Items) :
      (Object.prototype.hasOwnProperty.call(data, 'Items_json') ? String(data.Items_json || '[]') : (existing.Items_json || '[]'));
    // Recalcula Total sumando items
    let total = 0;
    try { (JSON.parse(itemsJson) || []).forEach(it => { total += (parseFloat(it.precio_total) || 0); }); } catch(_){}
    const row = INVENTARIOS_ORDENES_HEADERS.map(h => {
      if (h === 'ID') return id;
      if (h === 'Items_json') return itemsJson;
    ...
    if (isNew || !foundRow) sh.appendRow(row);
    else sh.getRange(foundRow, 1, 1, INVENTARIOS_ORDENES_HEADERS.length).setValues([row]);
    return { ok: true, ID: id, Total: total };
  } catch (err) { return { ok: false, error: String(err) }; }
}
function inventariosOrdenDelete_(data) {
  try {
    const id = String((data && data.ID) || '').trim();
    if (!id) return { ok: false, error: 'ID requerido' };
    const sh = _invEnsureSheet_(INVENTARIOS_ORDENES_SHEET, INVENTARIOS_ORDENES_HEADERS);
...
function inquilinosDriveRoot_() {
  // Carpeta Drive: Check Inn - Sistemas/Ticket vision/Código/Inquilinos
  var folder = DriveApp.getRootFolder();
  folder = getOrCreateFolder_(folder, 'Check Inn - Sistemas');
  folder = getOrCreateFolder_(folder, 'Ticket vision');
  folder = getOrCreateFolder_(folder, 'Código');
  folder = getOrCreateFolder_(folder, 'Inquilinos');
  return folder;
}

/** Escribe/repara los headers de la fila 1. Idempotente: si ya coinciden
 *  no hace nada. Si la hoja está vacía o los headers están incompletos,
 *  los rellena. Esto se auto-cura por si alguien creó la hoja a mano
 *  o si se agregaron columnas nuevas en versiones posteriores. */
function inquilinosEnsureHeaders_(sh, headers) {
  var lastCol = Math.max(sh.getLastColumn(), headers.length);
  var current = sh.getLastRow() >= 1 ? sh.getRange(1, 1, 1, lastCol).getValues()[0] : [];
  var needsWrite = false;
  for (var i = 0; i < headers.length; i++) {
    if (String(current[i] || '').trim() !== headers[i]) { needsWrite = true; break; }
  }
  if (needsWrite) {
    sh.getRange(1, 1, 1, headers.length).setValues([headers]);
...
function ensureInquilinosSheet_() {
  var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  var sh = ss.getSheetByName(INQUILINOS_SHEET);
  if (!sh) sh = ss.insertSheet(INQUILINOS_SHEET);
  inquilinosEnsureHeaders_(sh, INQUILINOS_HEADERS);
  return sh;
}

function ensureInquilinosPagosSheet_() {
  var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  var sh = ss.getSheetByName(INQUILINOS_PAGOS_SHEET);
  if (!sh) sh = ss.insertSheet(INQUILINOS_PAGOS_SHEET);
  inquilinosEnsureHeaders_(sh, INQUILINOS_PAGOS_HEADERS);
  return sh;
}

function inquilinosNewId_(prefix) {
  var d = new Date();
  var stamp = Utilities.formatDate(d, 'GMT-6', 'yyyyMMddHHmmss');
  var rnd = Math.floor(Math.random() * 9000 + 1000);
  return (prefix || 'INQ') + '-' + stamp + '-' + rnd;
}

function inquilinosComputeEstado_(fechaInicio, fechaFin) {
  try {
    if (!fechaFin) return String(fechaInicio || '') ? 'Vigente' : '';
    var now = new Date(); now.setHours(0,0,0,0);
    var fin = parseDateSafe_(fechaFin);
    if (!fin) return '';
    return fin < now ? 'Vencido' : 'Vigente';
  } catch(_) { return ''; }
}

function inquilinosSheetToObjects_(sh, headers) {
  var last = sh.getLastRow();
  if (last < 2) return [];
  ...
  return values.map(function(r) {
    var o = {};
    headers.forEach(function(h, i) { o[h] = r[i]; });
    // Deserializa los JSON de arrays de archivos
    ['Contrato_files_json','Identificacion_files_json','Aval_identificacion_files_json','CIF_files_json','Comprobante_files_json','Servicios_json','Muebles_json'].forEach(function(k){
      if (o[k]) { try { o[k.replace('_json','')] = JSON.parse(o[k]); } catch(_) { o[k.replace('_json','')] = []; } }
      else o[k.replace('_json','')] = [];
    });
    return o;
  });
}

function inquilinosList_() {
  try {
    var sh = ensureInquilinosSheet_();
    var rows = inquilinosSheetToObjects_(sh, INQUILINOS_HEADERS);
    // Recalcula Estado_contrato dinámicamente al leer.
    rows.forEach(function(r){
      var est = inquilinosComputeEstado_(r.Fecha_inicio, r.Fecha_fin);
      if (est) r.Estado_contrato = est;
    });
    return { ok: true, rows: rows };
  } catch (err) {
    Logger.log('[inquilinosList_] ' + err);
    return { ok: false, error: String(err) };
  }
}

function inquilinosSave_(data) {
  try {
    data = data || {};
    var sh = ensureInquilinosSheet_();
    var id = String(data.ID || '').trim();
    var isNew = !id;
    if (isNew) id = inquilinosNewId_('INQ');
    var now = new Date();
    var nowIso = now.toISOString();
    var estado = inquilinosComputeEstado_(data.Fecha_inicio, data.Fecha_fin);
    var contratoJson = JSON.stringify(Array.isArray(data.Contrato_files) ? data.Contrato_files : []);
    var identJson    = JSON.stringify(Array.isArray(data.Identificacion_files) ? data.Identificacion_files : []);
    var avalIdentJson= JSON.stringify(Array.isArray(data.Aval_identificacion_files) ? data.Aval_identificacion_files : []);
    var cifJson      = JSON.stringify(Array.isArray(data.CIF_files) ? data.CIF_files : []);
    var servJson     = JSON.stringify(data.Servicios || {});
    var muebJson     = JSON.stringify(data.Muebles || {});
    // Localiza row existente para preservar columnas no enviadas por el form.
    var foundRow = 0;
    var existing = {};
    if (!isNew) {
      var last = sh.getLastRow();
      if (last > 1) {
        var idsCol = sh.getRange(2, 1, last - 1, 1).getValues();
        for (var i = 0; i < idsCol.length; i++) {
          if (String(idsCol[i][0] || '').trim() === id) { foundRow = i + 2; break; }
        }
      }
      if (foundRow) {
        var curVals = sh.getRange(foundRow, 1, 1, INQUILINOS_HEADERS.length).getValues()[0];
        INQUILINOS_HEADERS.forEach(function(h, ix){ existing[h] = curVals[ix]; });
      }
    }

    // Los _json se sobrescriben SOLO si el array viene explícito en data;
    // si no, se preserva lo existente.
    var pick = function(key, jsonHeader, computed) {
      if (Object.prototype.hasOwnProperty.call(data, key)) return computed;
      return existing[jsonHeader] == null ? '[]' : String(existing[jsonHeader]);
    };
    var contratoOut  = pick('Contrato_files', 'Contrato_files_json', contratoJson);
    var identOut     = pick('Identificacion_files', 'Identificacion_files_json', identJson);
    var avalIdentOut = pick('Aval_identificacion_files', 'Aval_identificacion_files_json', avalIdentJson);
    var cifOut       = pick('CIF_files', 'CIF_files_json', cifJson);
    var servOut      = Object.prototype.hasOwnProperty.call(data, 'Servicios') ? servJson : (existing.Servicios_json == null ? '{}' : String(existing.Servicios_json));
    var muebOut      = Object.prototype.hasOwnProperty.call(data, 'Muebles')   ? muebJson : (existing.Muebles_json   == null ? '{}' : String(existing.Muebles_json));

    var row = INQUILINOS_HEADERS.map(function(h) {
      if (h === 'ID') return id;
      if (h === 'Estado_contrato') return estado || String(data.Estado_contrato != null ? data.Estado_contrato : (existing.Estado_contrato || ''));
      if (h === 'Contrato_files_json') return contratoOut;
      if (h === 'Identificacion_files_json') return identOut;
      if (h === 'Aval_identificacion_files_json') return avalIdentOut;
      if (h === 'CIF_files_json') return cifOut;
      if (h === 'Servicios_json') return servOut;
      if (h === 'Muebles_json') return muebOut;
      if (h === 'created_at') return isNew ? nowIso : (data.created_at || existing.created_at || nowIso);
      if (h === 'updated_at') return nowIso;
      // Save PARCIAL: usa data[h] si viene, si no preserva existente.
      if (Object.prototype.hasOwnProperty.call(data, h)) {
        return String(data[h] == null ? '' : data[h]);
      }
      return existing[h] == null ? '' : existing[h];
    });
    // Upsert por ID
    if (isNew || !foundRow) {
      ...
      sh.getRange(foundRow, 1, 1, INQUILINOS_HEADERS.length).setValues([row]);
    }
    return { ok: true, ID: id };
  } catch (err) {
    Logger.log('[inquilinosSave_] ' + err);
    return { ok: false, error: String(err) };
  }
}

function inquilinosDelete_(data) {
  try {
    ...
    var toDel = [];
    for (var i = 0; i < ids.length; i++) {
      if (String(ids[i][0] || '').trim() === id) toDel.push(i + 2);
    }
...
/** Sube un archivo (contrato o foto de ID) a la subcarpeta del inquilino.
 *  data: { inquilino_id, filename, mime, data (base64), kind: 'contrato'|'identificacion' } */
function inquilinosUploadFile_(data) {
  try {
    data = data || {};
    var iid = String(data.inquilino_id || '').trim() || 'sin_id';
    ...
    if (!b64) return { ok: false, error: 'data vacío' };
    var kind = String(data.kind || 'archivo').toLowerCase();
    var root = inquilinosDriveRoot_();
    var folder = getOrCreateFolder_(root, iid);
    folder = getOrCreateFolder_(folder, kind);
    ...
    Logger.log('[inquilinosUploadFile_] ' + err);
    return { ok: false, error: String(err) };
  }
}

// ── PAGOS de inquilinos ──
function inquilinosPagosList_(data) {
  try {
    data = data || {};
    var sh = ensureInquilinosPagosSheet_();
    var rows = inquilinosSheetToObjects_(sh, INQUILINOS_PAGOS_HEADERS);
    var iid = String(data.inquilino_id || '').trim();
    if (iid) rows = rows.filter(function(r){ return String(r.Inquilino_ID || '').trim() === iid; });
    // Orden desc por Mes
    rows.sort(function(a,b){ return String(b.Mes||'').localeCompare(String(a.Mes||'')); });
    return { ok: true, rows: rows };
  } catch (err) {
    return { ok: false, error: String(err) };
  }
...
function inquilinosPagoSave_(data) {
  try {
    data = data || {};
    var sh = ensureInquilinosPagosSheet_();
    var id = String(data.ID || '').trim();
    var isNew = !id;
    if (isNew) id = inquilinosNewId_('PAG');
    var now = new Date();
    ...
    if (!isNew) {
      ...
      if (foundRow) {
        var curVals = sh.getRange(foundRow, 1, 1, INQUILINOS_PAGOS_HEADERS.length).getValues()[0];
        INQUILINOS_PAGOS_HEADERS.forEach(function(h, ix){ existing[h] = curVals[ix]; });
      }
    }

    // Comprobante_files_json: se sobrescribe SOLO si viene explícito. Si el
    // frontend no envía Comprobante_files (undefined), preserva lo existente.
    var compJson;
    if (Object.prototype.hasOwnProperty.call(data, 'Comprobante_files')) {
      compJson = JSON.stringify(Array.isArray(data.Comprobante_files) ? data.Comprobante_files : []);
    } else if (existing.Comprobante_files_json != null) {
      compJson = String(existing.Comprobante_files_json);
    } else {
      compJson = '[]';
    }

    var row = INQUILINOS_PAGOS_HEADERS.map(function(h){
      if (h === 'ID') return id;
      if (h === 'created_at') return isNew ? now.toISOString() : (data.created_at || existing.created_at || now.toISOString());
      if (h === 'Comprobante_files_json') return compJson;
      // Save PARCIAL: si el field viene en data, úsalo; si no, preserva el
      // valor existente. Evita que un save del form (que sólo trae Nombre,
      // Mes, Monto, etc.) borre Folio_facturapi, Ticket_url, Fecha_emision,
      // Estatus, o cualquier otra columna no incluida en el form.
      if (Object.prototype.hasOwnProperty.call(data, h)) {
        return String(data[h] == null ? '' : data[h]);
      }
      return existing[h] == null ? '' : existing[h];
    ...
    if (isNew || !foundRow) {
      ...
      sh.getRange(foundRow, 1, 1, INQUILINOS_PAGOS_HEADERS.length).setValues([row]);
    }
    return { ok: true, ID: id };
  } catch (err) {
    return { ok: false, error: String(err) };
...
/** Writeback de Facturapi para pagos de inquilinos.
 *  externalId="INQPAGO-{ID}"; escribe Folio + Organizacion + Ticket_url si vienen. */
function inquilinosPagoUpdateFacturapi_(data) {
  try {
    var extRaw = String(data.external_id || data.externalId || data.record_id || data.id || '').trim();
    var id = extRaw.indexOf('INQPAGO-') === 0 ? extRaw.substring('INQPAGO-'.length) : extRaw;
    if (!id) return { ok:false, error:'Falta ID de pago' };
    var sh = ensureInquilinosPagosSheet_();
    var last = sh.getLastRow();
    if (last < 2) return { ok:false, error:'Hoja vacía' };
    var ids = sh.getRange(2, 1, last - 1, 1).getValues();
    ...
    if (!foundRow) return { ok:false, error:'Pago no encontrado' };
    var headers = INQUILINOS_PAGOS_HEADERS;
    var folio = String(data.folio_facturapi || data.folio || '').trim();
    if (folio && !isNaN(Number(folio))) {
      var c = headers.indexOf('Folio_facturapi');
      if (c >= 0) sh.getRange(foundRow, c + 1).setNumberFormat('@').setValue(folio);
    }
    var orgRaw = String(data.org_label || data.organizacion_facturapi || data.org || '').trim();
    if (orgRaw === '1') orgRaw = 'ACR'; else if (orgRaw === '2') orgRaw = 'ACL';
    if (orgRaw) {
      var oc = headers.indexOf('Organizacion_facturapi');
      if (oc >= 0) sh.getRange(foundRow, oc + 1).setNumberFormat('@').setValue(orgRaw);
    }
    var ticketUrl = String(data.ticket_url || data.pdf_url || '').trim();
    if (ticketUrl) {
      var tc = headers.indexOf('Ticket_facturapi_url');
      if (tc >= 0) sh.getRange(foundRow, tc + 1).setValue(ticketUrl);
    }
    var estatus = String(data.estatus || data.status || '').trim();
    if (estatus) {
      var ec = headers.indexOf('Estatus_facturapi');
      if (ec >= 0) sh.getRange(foundRow, ec + 1).setValue(estatus);
    } else if (folio) {
      var ec2 = headers.indexOf('Estatus_facturapi');
      if (ec2 >= 0) sh.getRange(foundRow, ec2 + 1).setValue('emitida');
    }
    if (folio) {
      var fc = headers.indexOf('Fecha_emision');
      if (fc >= 0) {
        var cur = String(sh.getRange(foundRow, fc + 1).getValue() || '').trim();
        if (!cur) sh.getRange(foundRow, fc + 1).setValue(new Date().toISOString());
      }
    }
    SpreadsheetApp.flush();
    return { ok:true, id:id, folio_facturapi:folio, target_column:'Folio_facturapi', sheet_name:sh.getName() };
  } catch (err) {
    Logger.log('[inquilinosPagoUpdateFacturapi_] ' + err);
    return { ok:false, error:String(err) };
  }
}

/** Archiva el folio actual (mueve a Folio_facturapi_antiguo) y limpia el actual. */
function inquilinosPagoArchiveFolio_(data) {
  try {
    var id = String((data && (data.record_id || data.id)) || '').replace(/^INQPAGO-/, '').trim();
    if (!id) return { ok:false, error:'Falta ID de pago' };
    var sh = ensureInquilinosPagosSheet_();
    var last = sh.getLastRow();
    if (last < 2) return { ok:false, error:'Hoja vacía' };
    ...
    if (!foundRow) return { ok:false, error:'Pago no encontrado' };
    var headers = INQUILINOS_PAGOS_HEADERS;
    var fCol = headers.indexOf('Folio_facturapi') + 1;
    var aCol = headers.indexOf('Folio_facturapi_antiguo') + 1;
    var tCol = headers.indexOf('Ticket_facturapi_url') + 1;
    var eCol = headers.indexOf('Estatus_facturapi') + 1;
    var feCol = headers.indexOf('Fecha_emision') + 1;
    var prev = String(sh.getRange(foundRow, fCol).getValue() || '').trim();
    if (prev && aCol) {
      var oldPrev = String(sh.getRange(foundRow, aCol).getValue() || '').trim();
      sh.getRange(foundRow, aCol).setValue(oldPrev ? (oldPrev + ', ' + prev) : prev);
    }
    if (fCol) sh.getRange(foundRow, fCol).setValue('');
    if (tCol) sh.getRange(foundRow, tCol).setValue('');
    if (eCol) sh.getRange(foundRow, eCol).setValue('');
    if (feCol) sh.getRange(foundRow, feCol).setValue('');
    SpreadsheetApp.flush();
    return { ok:true, previous_folio: prev };
  } catch (err) {
    Logger.log('[inquilinosPagoArchiveFolio_] ' + err);
    return { ok:false, error:String(err) };
  }
}

/** Guarda el PDF del ticket Facturapi en Drive y escribe las 5 columnas
 *  Ticket_facturapi_* en Inquilinos_Pagos. Análogo a saveFacturapiPdf_ pero
 *  para pagos de inquilinos. Ruta Drive: /Inquilinos/{Nombre}/Tickets Facturapi/. */
function inquilinosPagoSaveFacturapiPdf_(data) {
  try {
    var extRaw = String(data.external_id || data.externalId || data.record_id || data.id || '').trim();
    var pagoId = extRaw.indexOf('INQPAGO-') === 0 ? extRaw.substring('INQPAGO-'.length) : extRaw;
    if (!pagoId) return { ok:false, error:'Falta ID de pago' };
    var receiptId = String(data.receipt_id || data.receiptId || '');
    var folio = String(data.folio_facturapi || data.folio || '');
    var fileObj = data.file;
    if (!fileObj || !fileObj.base64) return { ok:false, error:'Sin PDF' };
    // Localizar row del pago
    var sh = ensureInquilinosPagosSheet_();
    var last = sh.getLastRow();
    if (last < 2) return { ok:false, error:'Hoja vacía' };
    var ids = sh.getRange(2, 1, last - 1, 1).getValues();
    var foundRow = 0;
    for (var i = 0; i < ids.length; i++) {
      if (String(ids[i][0] || '').trim() === pagoId) { foundRow = i + 2; break; }
    }
    if (!foundRow) return { ok:false, error:'Pago no encontrado' };
    var pagoRow = {};
    var vals = sh.getRange(foundRow, 1, 1, INQUILINOS_PAGOS_HEADERS.length).getValues()[0];
    INQUILINOS_PAGOS_HEADERS.forEach(function(h, ix){ pagoRow[h] = vals[ix]; });
    // Datos del inquilino para el path
    var inqId = String(pagoRow.Inquilino_ID || '').trim();
    var inqName = 'Sin nombre';
    if (inqId) {
      try {
        var shInq = ensureInquilinosSheet_();
        var lastInq = shInq.getLastRow();
        if (lastInq >= 2) {
          var idsInq = shInq.getRange(2, 1, lastInq - 1, 1).getValues();
          for (var j = 0; j < idsInq.length; j++) {
            if (String(idsInq[j][0] || '').trim() === inqId) {
              var nameCol = INQUILINOS_HEADERS.indexOf('Nombre') + 1;
              if (nameCol > 0) inqName = String(shInq.getRange(j + 2, nameCol).getValue() || 'Sin nombre');
              break;
            }
          }
        }
    ...
    var mes = String(pagoRow.Mes || '').replace(/[^0-9-]/g, '') || Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM');
    var pdfName = cleanFolderName_(folio ? 'ticket_facturapi_folio_' + folio : 'ticket_facturapi_' + receiptId) + '.pdf';
    var fileInfo = saveFacturapiPdfToDrive_(
      { fileName: pdfName, mimeType: fileObj.mimeType || 'application/pdf', base64: fileObj.base64 },
      ['Inquilinos', cleanFolderName_(inqName), mes, 'Tickets Facturapi']
    );
    // Escribir 5 columnas del ticket
    var writes = {
      'Ticket_facturapi_url': fileInfo.url,
      'Ticket_facturapi_id_archivo': fileInfo.id,
      'Ticket_facturapi_nombre_archivo': fileInfo.name,
      'Ticket_facturapi_carpeta_url': fileInfo.folder_url,
      'Ticket_facturapi_carpeta_ruta': fileInfo.folder_path,
    };
    Object.keys(writes).forEach(function(h){
      var c = INQUILINOS_PAGOS_HEADERS.indexOf(h) + 1;
      if (c > 0) sh.getRange(foundRow, c).setValue(writes[h]);
    });
    // Actualiza también folio + estatus si vienen
    if (folio) {
      var fc = INQUILINOS_PAGOS_HEADERS.indexOf('Folio_facturapi') + 1;
      if (fc > 0) sh.getRange(foundRow, fc).setNumberFormat('@').setValue(folio);
      var ec = INQUILINOS_PAGOS_HEADERS.indexOf('Estatus_facturapi') + 1;
      if (ec > 0 && !String(sh.getRange(foundRow, ec).getValue() || '').trim()) sh.getRange(foundRow, ec).setValue('emitida');
      var feC = INQUILINOS_PAGOS_HEADERS.indexOf('Fecha_emision') + 1;
      if (feC > 0 && !String(sh.getRange(foundRow, feC).getValue() || '').trim()) sh.getRange(foundRow, feC).setValue(new Date().toISOString());
    }
    SpreadsheetApp.flush();
    return {
      ok: true, id: pagoId, receipt_id: receiptId, folio_facturapi: folio,
      ticket_facturapi_url: fileInfo.url, file_id: fileInfo.id, file_name: fileInfo.name,
      folder_url: fileInfo.folder_url, folder_path: fileInfo.folder_path,
      sheet_name: sh.getName(),
    };
  } catch (err) {
    Logger.log('[inquilinosPagoSaveFacturapiPdf_] ' + err);
    return { ok:false, error: String(err) };
  }
}

function inquilinosPagoDelete_(data) {
  try {
    var id = String((data && data.ID) || '').trim();
    if (!id) return { ok: false, error: 'ID requerido' };
    var sh = ensureInquilinosPagosSheet_();
...
const LODGIFY_HEADERS = [
  ...
  // Pagos (Lodgify v2 · sync trae total_amount / amount_paid / amount_due /
  // transactions[]). PaymentStatus se deriva en el backend Cloud Run.
  "TotalAmount","AmountPaid","AmountDue","PaymentStatus","PaymentPolicy","TransactionsJSON",
...
function ensureLodgifySheet_() {
  ...
  if (!sh) {
    ...
    if (sh.getLastRow() === 0) {
      ...
      // Migración: si faltan columnas (ej. las de pago recién agregadas),
      // extender el header row para que el sync pueda escribirlas.
      var lastCol = sh.getLastColumn();
      var currentHeaders = sh.getRange(1, 1, 1, Math.max(lastCol, 1)).getValues()[0]
        .map(function (h) { return String(h || '').trim(); });
      var missing = LODGIFY_HEADERS.filter(function (h) { return currentHeaders.indexOf(h) < 0; });
      if (missing.length) {
        sh.getRange(1, currentHeaders.length + 1, 1, missing.length).setValues([missing]);
        sh.getRange(1, currentHeaders.length + 1, 1, missing.length).setFontWeight('bold').setBackground('#f1f5f9');
      }
    }
  }
  ...
  (rows || []).forEach(r => {
    ...
    if (!agg) {
      ...
      agg = {
        ...
        LineItems: [],
        // Pagos: mismo valor en cada row de un booking (redundante en fila,
        // pero necesario porque agregamos por Id).
        TotalAmount: Number(r.TotalAmount) || 0,
        AmountPaid: Number(r.AmountPaid) || 0,
        AmountDue: Number(r.AmountDue) || 0,
        PaymentStatus: r.PaymentStatus || "",
        PaymentPolicy: r.PaymentPolicy || "",
        TransactionsJSON: r.TransactionsJSON || "[]",
      };
      map.set(id, agg);
    }
...
function lodgifyBookingToRow_(b, nowIso, prevFirstSync) {
  return [
    ...
    Number(b.TotalAmount || 0), Number(b.AmountPaid || 0), Number(b.AmountDue || 0),
    String(b.PaymentStatus || ""), String(b.PaymentPolicy || ""), String(b.TransactionsJSON || "[]"),
...
var LODGIFY_BOOKINGS_URL = "https://api.check-inn.mx/lodgify-bookings-all";
 ...
 *    purge_orphans  (default TRUE) si true: marca Status='Deleted' para bookings
 *                   cuyo Id está en el sheet dentro del rango [from,to] pero YA NO
 *                   aparecen en el pull de Lodgify (fueron eliminados en Lodgify).
 *                   Safety: aborta el purge si detecta >20% huérfanos O >50 huérfanos
 *                   (probable pull incompleto). */
function syncLodgifyReservations_(data) {
  data = data || {};
  const full = String(data.full || "").toLowerCase() === "true" || data.full === true;
  // Default true — el sync se pensó como fuente de verdad. Se puede desactivar
  // pasando purge_orphans:false explícitamente.
  const purgeOrphans = data.purge_orphans === false || String(data.purge_orphans || "").toLowerCase() === "false"
    ? false : true;
  ...
  // FIX shift columnas: leer headers ACTUALES de la hoja y mapear cada
  // campo del row por NOMBRE — no por posición. Si columnas nuevas se
  // agregaron al final por migración, el orden físico difiere del array
  // LODGIFY_HEADERS y escribir por posición desalinea los valores.
  const sheetCols = sh.getLastColumn();
  const actualHeaders = sh.getRange(1, 1, 1, sheetCols).getValues()[0]
    .map(function (h) { return String(h || '').trim(); });
  // Índice por nombre → posición 0-based en la hoja física.
  const colIdxByName = {};
  actualHeaders.forEach(function (h, i) { if (h) colIdxByName[h] = i; });
  function _bookingToPhysicalRow(b, nowIsoLocal, prevFirstSync) {
    const declared = lodgifyBookingToRow_(b, nowIsoLocal, prevFirstSync);
    // declared está en el orden de LODGIFY_HEADERS. Reindexar por nombre.
    const phys = new Array(sheetCols).fill('');
    for (let i = 0; i < LODGIFY_HEADERS.length; i++) {
      const name = LODGIFY_HEADERS[i];
      const target = colIdxByName[name];
      if (typeof target === 'number') phys[target] = declared[i];
    }
    return phys;
  }
  bookings.forEach(b => {
    ...
    const arr = _bookingToPhysicalRow(b, nowIso, ex ? ex.firstSync : nowIso);
    if (ex) {
      sh.getRange(ex.row, 1, 1, sheetCols).setValues([arr]);
      updated++;
    } else {
      toAppend.push(arr);
      inserted++;
  ...
  if (toAppend.length) {
    sh.getRange(sh.getLastRow() + 1, 1, toAppend.length, sheetCols).setValues(toAppend);
  }
  ...
  // ═══ Detección de huérfanos (bookings eliminados en Lodgify) ═══
  // Un huésped que YA está en la hoja dentro del rango [from,to] pero cuyo Id
  // NO viene en el pull → fue eliminado en Lodgify. Lo marcamos Status='Deleted'
  // (el frontend filtra por defecto estado='Booked', así deja de aparecer).
  // No borramos la fila para conservar historial y permitir auditoría/restore.
  let orphaned = 0;
  let orphan_ids = [];
  let orphan_skipped_reason = null;
  if (purgeOrphans) {
    try {
      const pullIds = new Set();
      bookings.forEach(function(b) { pullIds.add(String(b.Id)); });
      const finalLastRow = sh.getLastRow();
      const colStatus = headers.indexOf("Status");
      const colArrival = headers.indexOf("DateArrival");
      const colDeparture = headers.indexOf("DateDeparture");
      if (finalLastRow >= 2 && colStatus >= 0 && colArrival >= 0 && colDeparture >= 0) {
        const allRows = sh.getRange(2, 1, finalLastRow - 1, LODGIFY_HEADERS.length).getValues();
        ...
        for (let i = 0; i < allRows.length; i++) {
          ...
          if (!id) continue;
          const dArr = String(r[colArrival] || '').trim();
          const dDep = String(r[colDeparture] || '').trim();
          if (!dArr && !dDep) continue;
          // Parseo tolerante YYYY-MM-DD (formato dominante en el sheet).
          const parseYmd = function(s) {
            if (!s) return 0;
          ...
          const arrMs = parseYmd(dArr);
          const depMs = parseYmd(dDep) || arrMs;
          if (!arrMs) continue;
          // Overlap con [from,to]
          if (!(arrMs <= toMs && depMs >= fromMs)) continue;
          inWindowCount++;
          if (pullIds.has(id)) continue; // sigue viva
          const status = String(r[colStatus] || '').trim();
          if (status === 'Deleted') continue; // ya marcada
          candidates.push({ rowIdx: i + 2, id: id, prevStatus: status });
        }
        // Safety: si el pull vino muy incompleto respecto al sheet, abortar
        // para no marcar reservas legítimas como eliminadas.
        const ratio = inWindowCount > 0 ? (candidates.length / inWindowCount) : 0;
        if (candidates.length > 50 || ratio > 0.2) {
          orphan_skipped_reason = 'safety_threshold_exceeded (' + candidates.length + ' huérfanos de ' + inWindowCount + ' en ventana, ratio ' + (ratio*100).toFixed(1) + '%)';
        } else {
          candidates.forEach(function(c) {
            sh.getRange(c.rowIdx, colStatus + 1).setValue('Deleted');
            orphan_ids.push(c.id);
          });
          orphaned = candidates.length;
        }
      }
    } catch (err) {
      orphan_skipped_reason = 'error: ' + (err.message || String(err));
    }
  }
...
function getLodgifyReservations_(data) {
  ...
  try {
    var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    var shCO = ss.getSheetByName("check_out");
    if (shCO && shCO.getLastRow() > 1) {
      var coHeaders = shCO.getRange(1, 1, 1, shCO.getLastColumn()).getValues()[0];
      var coValues  = shCO.getRange(2, 1, shCO.getLastRow() - 1, shCO.getLastColumn()).getValues();
      checkOuts = coValues.map(function(r){
        var o = {};
        coHeaders.forEach(function(h, i){ o[String(h)] = r[i]; });
        return o;
      });
    }
...
/** UPDATE bulk de filas en BANCOS por rowNum.
 *  data.updates = [{ rowNum, fields: {…}, clearClasif? }]
 *  Solo escribe las columnas listadas en fields (match case-insensitive contra
 *  headers). Si clearClasif=true, además borra CUENTA/SUBCUENTA/CATEGORIA/CONCEPTO
 *  (para que la clasificación auto se re-corra en la próxima). */
function bnUpdateRowsBulk_(data) {
  const updates = (data && data.updates) || [];
  if (!Array.isArray(updates) || !updates.length) return { ok: true, written: 0 };
  const ss = getSpreadsheet_();
  const sh = ss.getSheets().find(function(s){
    return String(s.getName() || "").trim().toUpperCase() === "BANCOS";
  });
  ...
  try {
    ...
    const idxByNorm = {};
    headers.forEach(function(h, i){ idxByNorm[_norm(h)] = i; });
    const clearCols = ["CUENTA","SUBCUENTA","CATEGORIA","CONCEPTO"].map(_norm);
    var written = 0;
    updates.forEach(function(u){
      if (!u || !u.rowNum) return;
      const row = Number(u.rowNum);
      if (row < 2) return;
      const range = sh.getRange(row, 1, 1, headers.length);
      const values = range.getValues()[0];
      const fields = u.fields || {};
      Object.keys(fields).forEach(function(k){
        const col = idxByNorm[_norm(k)];
        if (col == null) return;
        values[col] = fields[k];
      });
      if (u.clearClasif) {
        clearCols.forEach(function(cn){
          const col = idxByNorm[cn];
          if (col != null) values[col] = "";
        });
      }
...
/** DELETE de UN row en BANCOS por rowNum. Con lock para evitar
 *  races con inserts / updates concurrentes. */
function bnBancosDeleteRow_(data) {
  const rowNum = Number(data && data.rowNum);
...
/** DEDUP MASIVO en BANCOS.
 *  Agrupa filas por huella (Día ISO + DESCRIPCION normalizada + CARGO + ABONO
 *  + Monto + Cuenta bancaria). Si un grupo tiene >1 filas, mantiene la MEJOR
 *  y borra las demás.
 *
 *  Reglas de "mejor":
 *   1. Prefiere la que tenga CUENTA o CATEGORIA clasificada (no vacía).
 *   2. Empate → prefiere la de menor rowNum (primera insertada).
 *
 *  Parámetros (data):
 *   - onlyEfectivo: boolean (default true) — restringe a Cuenta bancaria = Efectivo
 *   - dryRun: boolean (default false) — solo reporta, no borra
 *
 *  Respuesta: { ok, totalRows, gruposDup, filasBorradas, dryRun, samples[] }
 */
function bnBancosDedupeMassive_(data) {
  const onlyEfectivo = (data && data.onlyEfectivo === false) ? false : true;
  ...
  const displ  = sh.getDataRange().getDisplayValues();
  if (values.length < 2) return { ok: true, totalRows: 0, gruposDup: 0, filasBorradas: 0 };
  ...
  var iDia = pickIdx(["DÍA","DIA","FECHA"]);
  var iDes = pickIdx(["DESCRIPCION","DESCRIPCIÓN"]);
  var iCar = pickIdx(["CARGO"]);
  var iAbo = pickIdx(["ABONO"]);
  var iMon = pickIdx(["MONTO"]);
  var iCta = pickIdx(["CUENTA BANCARIA"]);
  var iSub = pickIdx(["SUBCUENTA_BANCARIA","SUBCUENTA BANCARIA"]);
  var iCuentaClas   = pickIdx(["CUENTA"]);      // clasificación contable
  var iCategoria    = pickIdx(["CATEGORIA","CATEGORÍA"]);
  var iConcepto     = pickIdx(["CONCEPTO"]);
  if (iDia < 0 || iDes < 0) return { ok: false, error: "Faltan columnas DÍA o DESCRIPCION." };
  var norm = function(s){
    return String(s || "")
      .normalize("NFD").replace(/[̀-ͯ]/g, "")
      .replace(/[ ]/g, " ")
  ...
  var toIso = function(v, disp){
    // Preferir DisplayValue para evitar problemas de timezone en cells Date.
    var s = String(disp || v || "").trim();
    if (!s) return "";
    var m = s.match(/^(\d{4})-(\d{2})-(\d{2})/);
    if (m) return m[1] + "-" + m[2] + "-" + m[3];
    m = s.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})/);
    if (m) return m[3] + "-" + String(m[2]).padStart(2, "0") + "-" + String(m[1]).padStart(2, "0"); // DD/MM/YYYY (locale es-MX)
    return s;
  };
  var isEfectivo = function(row){
    var ct = norm(row[iCta]);
    return ct === "efectivo" || ct.indexOf("efectivo") >= 0;
  };
  // Agrupar por huella
  var groups = {};
  for (var r = 1; r < values.length; r++) {
    var row = values[r];
    if (onlyEfectivo && iCta >= 0 && !isEfectivo(row)) continue;
    var dia = toIso(row[iDia], displ[r][iDia]);
    var des = norm(row[iDes]);
    var car = numStr(row[iCar]);
    var abo = numStr(row[iAbo]);
    var mon = numStr(row[iMon]);
    var cta = norm(row[iCta]);
    if (!dia || !des) continue;
    // Huella EXCLUYE la clasificación (para que dup con clasificaciones
    // distintas caiga en el mismo grupo — es la misma transacción).
    var key = [dia, des, car, abo, mon, cta].join("|");
    if (!groups[key]) groups[key] = [];
    groups[key].push({
      rowNum: r + 1, // 1-indexed en Sheets
      hasCuenta:  iCuentaClas   >= 0 && String(row[iCuentaClas]   || "").trim() !== "",
      hasCat:     iCategoria    >= 0 && String(row[iCategoria]    || "").trim() !== "",
      hasConcep:  iConcepto     >= 0 && String(row[iConcepto]     || "").trim() !== "",
      hasSub:     iSub          >= 0 && String(row[iSub]          || "").trim() !== "",
      desc: String(row[iDes] || ""),
      dia: dia, monto: mon,
    });
  }
  var samples = [];
  var toDelete = []; // rowNums a borrar
  var gruposDup = 0;
  Object.keys(groups).forEach(function(k){
    var g = groups[k];
    if (g.length < 2) return;
    gruposDup++;
    // Ordenar: mejor primero (más clasificación, luego menor rowNum)
    g.sort(function(a, b){
      var scoreA = (a.hasCuenta?4:0) + (a.hasCat?2:0) + (a.hasConcep?1:0);
      var scoreB = (b.hasCuenta?4:0) + (b.hasCat?2:0) + (b.hasConcep?1:0);
      if (scoreA !== scoreB) return scoreB - scoreA;
      return a.rowNum - b.rowNum;
    });
    // Mantener g[0], borrar resto
    for (var i = 1; i < g.length; i++) toDelete.push(g[i].rowNum);
    if (samples.length < 15) {
      samples.push({
        key: k.slice(0, 80),
        keepRow: g[0].rowNum,
        deleteRows: g.slice(1).map(function(x){ return x.rowNum; }),
        desc: g[0].desc,
        dia: g[0].dia,
        monto: g[0].monto,
      });
  ...
  if (dryRun || !toDelete.length) {
    return {
      ...
      filasBorradas: 0,
      dryRun: true,
      previewCount: toDelete.length,
      samples: samples,
    };
  }
  // Borrar de MAYOR a MENOR rowNum para no invalidar índices.
...
function getBancosData_(ss) {
  ...
  const iCargo  = pickIdx(hB, ["CARGO"]);
  const iAbono  = pickIdx(hB, ["ABONO"]);
  const iSaldo  = pickIdx(hB, ["SALDO"]);
  ...
  // Módulo Efectivo: ORIGEN/DESTINO + comentarios. Sin estos, el frontend
  // muestra campos vacíos en la tabla de "Carga de movimientos de efectivo"
  ...
  const records = bancos.slice(1)
    ...
    .map(({ r, rowNum }) => {
      ...
      return {
        ...
        CARGO:             iCargo >= 0 ? (r[iCargo] === "" || r[iCargo] == null ? "" : toNumber(r[iCargo])) : "",
        ABONO:             iAbono >= 0 ? (r[iAbono] === "" || r[iAbono] == null ? "" : toNumber(r[iAbono])) : "",
        SALDO:             iSaldo >= 0 ? (r[iSaldo] === "" || r[iSaldo] == null ? "" : toNumber(r[iSaldo])) : "",
...
// ═══════════════════════════════════════════════════════════════════════════
// ║ Alojamientos — editar catálogo desde admin (guías de bienvenida)        ║
// ═══════════════════════════════════════════════════════════════════════════
function saveAlojamiento_(data) {
  try {
    var payload = data.payload ? (typeof data.payload === 'string' ? JSON.parse(data.payload) : data.payload) : data;
    var houseId = String(payload.HouseId || payload.houseId || payload.ID || '').trim();
    if (!houseId) return { ok: false, error: 'HouseId requerido.' };
    var changes = payload.changes || {};
    if (typeof changes === 'string') changes = JSON.parse(changes);
    if (!changes || typeof changes !== 'object' || !Object.keys(changes).length) return { ok: false, error: 'Sin cambios.' };
    var ss = getSpreadsheet_();
    var sh = ss.getSheetByName(ALOJAMIENTOS_SHEET);
    if (!sh) return { ok: false, error: 'Hoja "alojamientos" no existe.' };
    // Asegura que todas las columnas de changes existan como headers.
    var existing = sh.getRange(1, 1, 1, Math.max(1, sh.getLastColumn())).getValues()[0].map(function (v) { return String(v || '').trim(); });
    var missing = Object.keys(changes).filter(function (k) { return existing.indexOf(k) === -1; });
    if (missing.length) {
      var startCol = existing.length + 1;
      sh.getRange(1, startCol, 1, missing.length).setValues([missing]).setFontWeight('bold').setBackground('#dbeafe');
      existing = existing.concat(missing);
    }
    // Localiza la fila con HouseId matching.
    var idxHouse = existing.indexOf('HouseId') + 1;
    if (!idxHouse) return { ok: false, error: 'Columna HouseId no existe en la hoja.' };
    var lastRow = sh.getLastRow();
    if (lastRow < 2) return { ok: false, error: 'Sin filas de datos.' };
    var ids = sh.getRange(2, idxHouse, lastRow - 1, 1).getDisplayValues();
    var targetRow = -1;
    for (var i = 0; i < ids.length; i++) {
      if (String(ids[i][0]).trim() === houseId) { targetRow = i + 2; break; }
    }
    if (targetRow < 0) return { ok: false, error: 'No se encontró alojamiento con HouseId ' + houseId };
    // Aplica cambios columna por columna.
    var written = 0;
    for (var k in changes) {
      var col = existing.indexOf(k) + 1;
      if (col) { sh.getRange(targetRow, col).setValue(changes[k] == null ? '' : String(changes[k])); written++; }
    }
    return { ok: true, houseId: houseId, updated: written };
  } catch (err) {
    return { ok: false, error: String(err && err.message || err) };
  }
}
...
// ═══════════════════════════════════════════════════════════════════════════
// ║ MÓDULO WhatsApp automation — config por reserva + log de mensajes       ║
// ║ Hojas: WA_Config, WA_Log (auto-creadas).                                 ║
// ═══════════════════════════════════════════════════════════════════════════

var WA_CONFIG_HEADERS = ["Booking_ID","Auto_enabled","Updated_at","Updated_by","Disabled_templates","Recipients"];
var WA_LOG_HEADERS    = ["Timestamp","Booking_ID","Tipo","Origin","To","From","SID","Status","Body_preview"];

function waEnsureSheet_(name, headers) {
  var ss = getSpreadsheet_();
  ...
  if (!sh) {
    ...
    // Auto-heal headers si están vacíos o distintos
    var row1 = sh.getRange(1, 1, 1, Math.max(headers.length, sh.getLastColumn() || headers.length)).getValues()[0];
    var needsWrite = false;
    for (var i = 0; i < headers.length; i++) {
      if ((row1[i] || "").toString().trim() !== headers[i]) { needsWrite = true; break; }
    }
    if (needsWrite) sh.getRange(1, 1, 1, headers.length).setValues([headers]);
  }
  return sh;
}

function waConfigGetBatch_(data) {
  var ids = (data && data.booking_ids) || [];
  if (!Array.isArray(ids)) ids = [];
  var idsSet = {};
  for (var i = 0; i < ids.length; i++) idsSet[String(ids[i])] = true;
  var sh = waEnsureSheet_("WA_Config", WA_CONFIG_HEADERS);
  var last = sh.getLastRow();
  var map = {};
  if (last >= 2) {
    var vals = sh.getRange(2, 1, last - 1, WA_CONFIG_HEADERS.length).getValues();
    for (var r = 0; r < vals.length; r++) {
      var id = String(vals[r][0] || "").trim();
      if (!id) continue;
      if (ids.length && !idsSet[id]) continue;
      var disabledRaw = String(vals[r][4] || "").trim();
      var disabled = [];
      if (disabledRaw) {
        try { disabled = JSON.parse(disabledRaw); if (!Array.isArray(disabled)) disabled = []; }
        catch(_) { disabled = disabledRaw.split(",").map(function(s){return s.trim();}).filter(Boolean); }
      }
      var recipientsRaw = String(vals[r][5] || "").trim();
      var recipients = [];
      if (recipientsRaw) {
        try { recipients = JSON.parse(recipientsRaw); if (!Array.isArray(recipients)) recipients = []; }
        catch(_) { recipients = recipientsRaw.split(",").map(function(s){return s.trim();}).filter(Boolean); }
      }
      map[id] = {
        auto_enabled: /^(sí|si|yes|true|1)$/i.test(String(vals[r][1] || "")),
        updated_at: vals[r][2] ? new Date(vals[r][2]).toISOString() : "",
        updated_by: String(vals[r][3] || ""),
        disabled_templates: disabled,
        recipients: recipients,
      };
    }
  }
  return { ok: true, config: map };
}

function waConfigSet_(data) {
  var bookingId = String((data && data.booking_id) || "").trim();
  if (!bookingId) return { ok: false, error: "booking_id requerido" };
  var by = String((data && data.updated_by) || "admin");
  var sh = waEnsureSheet_("WA_Config", WA_CONFIG_HEADERS);
  var last = sh.getLastRow();
  var rowIdx = -1;
  var existingAuto = false;
  var existingDisabled = "";
  var existingRecipients = "";
  if (last >= 2) {
    var vals = sh.getRange(2, 1, last - 1, WA_CONFIG_HEADERS.length).getValues();
    for (var i = 0; i < vals.length; i++) {
      if (String(vals[i][0] || "").trim() === bookingId) {
        rowIdx = i + 2;
        existingAuto = /^(sí|si|yes|true|1)$/i.test(String(vals[i][1] || ""));
        existingDisabled = String(vals[i][4] || "");
        existingRecipients = String(vals[i][5] || "");
        break;
      }
    }
  }
  var enabled = (data && Object.prototype.hasOwnProperty.call(data, "auto_enabled"))
    ? !!data.auto_enabled : existingAuto;
  var disabledStr = existingDisabled;
  if (data && Object.prototype.hasOwnProperty.call(data, "disabled_templates")) {
    var arr = data.disabled_templates;
    if (typeof arr === "string") { try { arr = JSON.parse(arr); } catch(_) { arr = []; } }
    if (!Array.isArray(arr)) arr = [];
    disabledStr = JSON.stringify(arr);
  }
  var recipientsStr = existingRecipients;
  if (data && Object.prototype.hasOwnProperty.call(data, "recipients")) {
    var rec = data.recipients;
    if (typeof rec === "string") { try { rec = JSON.parse(rec); } catch(_) { rec = []; } }
    if (!Array.isArray(rec)) rec = [];
    recipientsStr = JSON.stringify(rec);
  }
  var row = [bookingId, enabled ? "Sí" : "No", new Date(), by, disabledStr, recipientsStr];
  if (rowIdx > 0) {
    sh.getRange(rowIdx, 1, 1, WA_CONFIG_HEADERS.length).setValues([row]);
  } else {
    sh.appendRow(row);
  }
  return { ok: true, booking_id: bookingId, auto_enabled: enabled, disabled_templates: disabledStr, recipients: recipientsStr };
}

function waLogAdd_(data) {
  var sh = waEnsureSheet_("WA_Log", WA_LOG_HEADERS);
  var row = [
    new Date(),
    String((data && data.booking_id) || ""),
    String((data && data.tipo) || ""),
    String((data && data.origin) || ""),
    String((data && data.to) || ""),
    String((data && data.from) || ""),
    String((data && data.sid) || ""),
    String((data && data.status) || ""),
    String((data && data.body_preview) || "").slice(0, 300),
  ];
  sh.appendRow(row);
  return { ok: true };
}

function waLogGetBatch_(data) {
  var ids = (data && data.booking_ids) || [];
  if (!Array.isArray(ids)) ids = [];
  var idsSet = {};
  for (var i = 0; i < ids.length; i++) idsSet[String(ids[i])] = true;
  var limit = Number((data && data.limit_per_booking) || 5);
  var sh = waEnsureSheet_("WA_Log", WA_LOG_HEADERS);
  var last = sh.getLastRow();
  var map = {};
  if (last >= 2) {
    // Leemos todo y recorremos del más nuevo al más viejo (últimas rows).
    var vals = sh.getRange(2, 1, last - 1, WA_LOG_HEADERS.length).getValues();
    for (var r = vals.length - 1; r >= 0; r--) {
      var bid = String(vals[r][1] || "").trim();
      if (!bid) continue;
      if (ids.length && !idsSet[bid]) continue;
      if (!map[bid]) map[bid] = [];
      if (map[bid].length >= limit) continue;
      map[bid].push({
        timestamp: vals[r][0] ? new Date(vals[r][0]).toISOString() : "",
        tipo: String(vals[r][2] || ""),
        origin: String(vals[r][3] || ""),
        to: String(vals[r][4] || ""),
        sid: String(vals[r][6] || ""),
        status: String(vals[r][7] || ""),
        body_preview: String(vals[r][8] || "").slice(0, 120),
      });
    }
  }
  return { ok: true, logs: map };
}

// ═══════════════════════════════════════════════════════════════════════════
// ║ WA — extras: URL guía desde alojamientos + mensajes programados custom  ║
// ═══════════════════════════════════════════════════════════════════════════

var WA_SCHEDULED_HEADERS = ["ID","Created_at","Booking_ID","Tipo","Scheduled_at","Body","To","Status","Sent_at","SID","Created_by","Asunto"];

/** Devuelve url_guia desde la hoja "alojamientos" para un HouseId dado. */
function waUrlGuiaGet_(data) {
  var houseId = String((data && data.house_id) || "").trim();
  if (!houseId) return { ok: false, error: "house_id requerido" };
  var ss = getSpreadsheet_();
  var sh = ss.getSheetByName(ALOJAMIENTOS_SHEET);
  if (!sh) return { ok: false, error: "hoja alojamientos no existe" };
  var vals = sh.getDataRange().getValues();
  if (vals.length < 2) return { ok: true, url_guia: "" };
  var headers = vals[0].map(function(h){ return String(h||"").trim(); });
  var iId  = headers.indexOf("HouseId");
  var iUrl = headers.indexOf("url_guia");
  if (iId < 0) return { ok: false, error: "columna HouseId no existe" };
  for (var r = 1; r < vals.length; r++) {
    if (String(vals[r][iId]||"").trim() === houseId) {
      return { ok: true, url_guia: iUrl >= 0 ? String(vals[r][iUrl]||"").trim() : "" };
    }
  }
  return { ok: true, url_guia: "" };
}

function waScheduledAdd_(data) {
  var sh = waEnsureSheet_("WA_Scheduled", WA_SCHEDULED_HEADERS);
  var id = "S" + new Date().getTime() + "_" + Math.floor(Math.random()*10000);
  var row = [
    id,
    new Date(),
    String((data && data.booking_id) || ""),
    String((data && data.tipo) || "custom"),
    String((data && data.scheduled_at) || ""),
    String((data && data.body) || "").slice(0, 4000),
    String((data && data.to) || ""),
    "pending",
    "",
    "",
    String((data && data.created_by) || "admin"),
    String((data && data.asunto) || "").slice(0, 100),
  ];
  sh.appendRow(row);
  return { ok: true, id: id };
}

function waScheduledDelete_(data) {
  var id = String((data && data.id) || "").trim();
  if (!id) return { ok: false, error: "id requerido" };
  var sh = waEnsureSheet_("WA_Scheduled", WA_SCHEDULED_HEADERS);
  var row = _waScheduledFindRow_(sh, id);
  if (row < 0) return { ok: false, error: "no encontrado" };
  sh.deleteRow(row);
  return { ok: true, id: id };
}

function waScheduledList_(data) {
  var bookingId = String((data && data.booking_id) || "").trim();
  var sh = waEnsureSheet_("WA_Scheduled", WA_SCHEDULED_HEADERS);
  var last = sh.getLastRow();
  var items = [];
  if (last >= 2) {
    var vals = sh.getRange(2, 1, last-1, WA_SCHEDULED_HEADERS.length).getValues();
    for (var r = 0; r < vals.length; r++) {
      var bid = String(vals[r][2]||"").trim();
      if (bookingId && bid !== bookingId) continue;
      items.push({
        id: String(vals[r][0]||""),
        created_at: vals[r][1] ? new Date(vals[r][1]).toISOString() : "",
        booking_id: bid,
        tipo: String(vals[r][3]||""),
        scheduled_at: String(vals[r][4]||""),
        body: String(vals[r][5]||""),
        to: String(vals[r][6]||""),
        status: String(vals[r][7]||""),
        sent_at: vals[r][8] ? new Date(vals[r][8]).toISOString() : "",
        sid: String(vals[r][9]||""),
        created_by: String(vals[r][10]||""),
        asunto: String(vals[r][11]||""),
      });
    }
  }
  return { ok: true, items: items };
...
function _waScheduledFindRow_(sh, id) {
  var last = sh.getLastRow();
  if (last < 2) return -1;
  var ids = sh.getRange(2, 1, last-1, 1).getValues();
  for (var i = 0; i < ids.length; i++) {
...
function waScheduledOmit_(data) {
  var id = String((data && data.id) || "").trim();
  if (!id) return { ok: false, error: "id requerido" };
  var sh = waEnsureSheet_("WA_Scheduled", WA_SCHEDULED_HEADERS);
  var row = _waScheduledFindRow_(sh, id);
  if (row < 0) return { ok: false, error: "no encontrado" };
  sh.getRange(row, 8).setValue("omitted");
  return { ok: true, id: id };
}

function waScheduledMarkSent_(data) {
  var id = String((data && data.id) || "").trim();
  if (!id) return { ok: false, error: "id requerido" };
  var sh = waEnsureSheet_("WA_Scheduled", WA_SCHEDULED_HEADERS);
  var row = _waScheduledFindRow_(sh, id);
  if (row < 0) return { ok: false, error: "no encontrado" };
  sh.getRange(row, 8).setValue(String(data.status || "sent"));
  sh.getRange(row, 9).setValue(new Date());
  sh.getRange(row, 10).setValue(String(data.sid || ""));
  return { ok: true, id: id };
}

/** Actualiza campos de un scheduled: body, scheduled_at, y/o status.
 *  Si status pasa a "pending", limpia sent_at y sid (reactivación). */
function waScheduledUpdate_(data) {
  var id = String((data && data.id) || "").trim();
  if (!id) return { ok: false, error: "id requerido" };
  var sh = waEnsureSheet_("WA_Scheduled", WA_SCHEDULED_HEADERS);
  var row = _waScheduledFindRow_(sh, id);
  ...
  if (data && Object.prototype.hasOwnProperty.call(data, "body")) {
    sh.getRange(row, 6).setValue(String(data.body || "").slice(0, 4000));
    updated.body = true;
  }
  if (data && Object.prototype.hasOwnProperty.call(data, "scheduled_at")) {
    sh.getRange(row, 5).setValue(String(data.scheduled_at || ""));
    updated.scheduled_at = true;
  }
  if (data && Object.prototype.hasOwnProperty.call(data, "status")) {
    var st = String(data.status || "");
    sh.getRange(row, 8).setValue(st);
    if (st === "pending") {
      sh.getRange(row, 9).setValue("");   // limpia sent_at
      sh.getRange(row, 10).setValue("");  // limpia sid
    }
    updated.status = st;
  }
...
/** Lista todos los pending con scheduled_at <= now (para cron). */
function waScheduledPending_(data) {
  var sh = waEnsureSheet_("WA_Scheduled", WA_SCHEDULED_HEADERS);
  var last = sh.getLastRow();
  var now = new Date();
  var items = [];
  if (last >= 2) {
    var vals = sh.getRange(2, 1, last-1, WA_SCHEDULED_HEADERS.length).getValues();
    for (var r = 0; r < vals.length; r++) {
      if (String(vals[r][7]||"").toLowerCase() !== "pending") continue;
      var schStr = String(vals[r][4]||"").trim();
      if (!schStr) continue;
      var sch = new Date(schStr);
      if (isNaN(sch.getTime())) continue;
      if (sch > now) continue; // aún no toca
      items.push({
        id: String(vals[r][0]||""),
        booking_id: String(vals[r][2]||""),
        tipo: String(vals[r][3]||""),
        scheduled_at: schStr,
        body: String(vals[r][5]||""),
        to: String(vals[r][6]||""),
...
// ═══════════════════════════════════════════════════════════════════════════
// ║ WA — plantillas de mensajes programados (Configuración Admin)           ║
// ═══════════════════════════════════════════════════════════════════════════

var WA_TEMPLATES_HEADERS = ["ID","Nombre","Body","Asunto","Schedule_type","Schedule_time","Alojamientos","Enabled","Created_at","Updated_at","Updated_by","Schedule_event","Schedule_offset","Responsivo","Placeholders_custom"];

function _waTplTimeToStr_(v) {
  if (!v && v !== 0) return "";
  if (v instanceof Date) {
...
function waTemplatesList_() {
  var sh = waEnsureSheet_("WA_Templates", WA_TEMPLATES_HEADERS);
  // Auto-migración: si a la hoja existente le falta alguna columna nueva, la
  // agrega al final sin tocar las existentes.
  _waTemplatesMigrateHeaders_(sh);
  var last = sh.getLastRow();
  var items = [];
  if (last >= 2) {
    var vals = sh.getRange(2, 1, last-1, WA_TEMPLATES_HEADERS.length).getValues();
    for (var r = 0; r < vals.length; r++) {
      var enabledRaw = vals[r][7];
      var enabled = enabledRaw === true || String(enabledRaw).toUpperCase() === "TRUE";
      var respRaw = vals[r][13];
      var responsivo = respRaw === true || String(respRaw).toUpperCase() === "TRUE";
      items.push({
        id: String(vals[r][0]||""),
        nombre: String(vals[r][1]||""),
        body: String(vals[r][2]||""),
        asunto: String(vals[r][3]||""),
        schedule_type: String(vals[r][4]||""),
        schedule_time: _waTplTimeToStr_(vals[r][5]),
        alojamientos: String(vals[r][6]||""),
        enabled: enabled,
        created_at: vals[r][8] ? new Date(vals[r][8]).toISOString() : "",
        updated_at: vals[r][9] ? new Date(vals[r][9]).toISOString() : "",
        updated_by: String(vals[r][10]||""),
        schedule_event: String(vals[r][11]||""),
        schedule_offset: (vals[r][12] === "" || vals[r][12] == null) ? "" : String(vals[r][12]),
        responsivo: responsivo,
        placeholders_custom: String(vals[r][14]||""),
      });
    }
  }
  return { ok: true, items: items };
...
/** Auto-migra headers de WA_Templates: agrega columnas nuevas al final si
 *  faltan. Idempotente. */
function _waTemplatesMigrateHeaders_(sh) {
  if (!sh) return;
  var lastCol = Math.max(sh.getLastColumn(), 1);
  var current = sh.getRange(1, 1, 1, lastCol).getValues()[0].map(function(v){ return String(v||"").trim(); });
  var missing = [];
  for (var i = 0; i < WA_TEMPLATES_HEADERS.length; i++) {
    if (current.indexOf(WA_TEMPLATES_HEADERS[i]) < 0) missing.push(WA_TEMPLATES_HEADERS[i]);
  }
  if (!missing.length) return;
  sh.getRange(1, lastCol + 1, 1, missing.length)
    .setValues([missing])
...
function waTemplatesUpsert_(data) {
  var sh = waEnsureSheet_("WA_Templates", WA_TEMPLATES_HEADERS);
  _waTemplatesMigrateHeaders_(sh);
  var id = String((data && data.id) || "").trim();
  var nombre = String((data && data.nombre) || "").trim();
  if (!nombre) return { ok: false, error: "nombre requerido" };
  var body = String((data && data.body) || "").slice(0, 4000);
  var asunto = String((data && data.asunto) || "").slice(0, 100);
  var scheduleType = String((data && data.schedule_type) || "never");
  var scheduleTime = String((data && data.schedule_time) || "");
  var alojamientos = String((data && data.alojamientos) || "");
  var enabled = data && data.enabled === true;
  var updatedBy = String((data && data.updated_by) || "admin");
  var scheduleEvent = String((data && data.schedule_event) || "");
  var scheduleOffset = (data && (data.schedule_offset || data.schedule_offset === 0)) ? String(data.schedule_offset) : "";
  var responsivo = data && data.responsivo === true;
  // Placeholders personalizados por template: guardado como JSON string
  // (array de {name, value}). Front lo envía ya serializado.
  var phCustom = "";
  if (data && data.placeholders_custom != null) {
    phCustom = (typeof data.placeholders_custom === "string")
      ? data.placeholders_custom
      : JSON.stringify(data.placeholders_custom);
  }
  var now = new Date();
  var last = sh.getLastRow();
  var textFmt = [["@","@","@","@","@","@","@","@","@","@","@","@","@","@","@"]];
  if (id && last >= 2) {
    ...
    for (var i = 0; i < ids.length; i++) {
      if (String(ids[i][0]||"").trim() === id) {
        var row = i + 2;
        var existing = sh.getRange(row, 1, 1, WA_TEMPLATES_HEADERS.length).getValues()[0];
        var range = sh.getRange(row, 1, 1, WA_TEMPLATES_HEADERS.length);
        range.setNumberFormats(textFmt);
        // Si el payload no trae placeholders_custom, preservar el valor previo.
        var phFinal = (data && data.placeholders_custom != null) ? phCustom : String(existing[14]||"");
        range.setValues([[
          id, nombre, body, asunto, scheduleType, scheduleTime, alojamientos, enabled, existing[8] || now, now, updatedBy, scheduleEvent, scheduleOffset, responsivo, phFinal
        ]]);
        return { ok: true, id: id, updated: true };
      }
    }
  }
  if (!id) id = "T" + now.getTime() + "_" + Math.floor(Math.random()*10000);
  var newRow = sh.getLastRow() + 1;
  var newRange = sh.getRange(newRow, 1, 1, WA_TEMPLATES_HEADERS.length);
  newRange.setNumberFormats(textFmt);
  newRange.setValues([[id, nombre, body, asunto, scheduleType, scheduleTime, alojamientos, enabled, now, now, updatedBy, scheduleEvent, scheduleOffset, responsivo, phCustom]]);
  return { ok: true, id: id, created: true };
}

function waTemplatesDelete_(data) {
  var id = String((data && data.id) || "").trim();
  if (!id) return { ok: false, error: "id requerido" };
  var sh = waEnsureSheet_("WA_Templates", WA_TEMPLATES_HEADERS);
  var last = sh.getLastRow();
...
// ═══════════════════════════════════════════════════════════════════════════
// ║ REPORTES TÉCNICOS (MVP F1)                                              ║
// ║ Hoja Reportes_Tecnicos. Folio auto RT-YYYY-NNNNNN. Estados / prioridad  ║
// ║ / responsabilidad como strings (validación en front).                   ║
// ═══════════════════════════════════════════════════════════════════════════
var RT_SHEET = "Reportes_Tecnicos";
var RT_HEADERS = [
  "ID","Folio","Fecha","Timestamp","Estado","Prioridad","Tipo","Categoria",
  "Propiedad","# Departamento","Alojamiento","Activo","Titulo","Descripcion",
  "Descripcion_solucion","Reportado_por","Asignado_a","Proveedor",
  "Fecha_compromiso","Bloquea_habitabilidad","Reincidente","Responsabilidad",
  "Costo_total","Cargar_a_huesped","Motivo_espera","Motivo_cancelacion",
  "Motivo_reapertura","Reservacion_id","Huesped_nombre","Huesped_contacto",
  "Fotos_antes_urls","Fotos_despues_urls","Notas","Updated_at","Updated_by"
];
function _rtEnsureSheet_() {
  var ss = getSpreadsheet_();
  var sh = ss.getSheetByName(RT_SHEET);
  if (!sh) {
    sh = ss.insertSheet(RT_SHEET);
    sh.getRange(1,1,1,RT_HEADERS.length).setValues([RT_HEADERS])
      .setFontWeight("bold").setBackground("#f1f5f9");
    sh.setFrozenRows(1);
    return sh;
  }
  // Auto-migración headers faltantes
  var lastCol = Math.max(sh.getLastColumn(),1);
  var cur = sh.getRange(1,1,1,lastCol).getValues()[0].map(function(v){return String(v||"").trim();});
  var missing = [];
  for (var i=0;i<RT_HEADERS.length;i++) if (cur.indexOf(RT_HEADERS[i])<0) missing.push(RT_HEADERS[i]);
  if (missing.length) sh.getRange(1,lastCol+1,1,missing.length).setValues([missing])
    .setFontWeight("bold").setBackground("#f1f5f9");
  return sh;
}
function _rtRowToObj_(row) {
  var o = {};
  for (var i=0;i<RT_HEADERS.length;i++) {
    var k = RT_HEADERS[i];
    var v = row[i];
    if (v instanceof Date) v = v.toISOString();
    o[k] = v == null ? "" : String(v);
  }
  o.Bloquea_habitabilidad = String(row[19]).toUpperCase() === "TRUE";
  o.Reincidente = String(row[20]).toUpperCase() === "TRUE";
  o.Cargar_a_huesped = String(row[23]).toUpperCase() === "TRUE";
  o.Costo_total = Number(row[22]) || 0;
  return o;
}
function rtList_(data) {
  var sh = _rtEnsureSheet_();
  var last = sh.getLastRow();
  if (last < 2) return { ok:true, rows: [] };
  var vals = sh.getRange(2,1,last-1,RT_HEADERS.length).getValues();
  var rows = [];
  for (var i=0;i<vals.length;i++) {
    if (!vals[i][0]) continue;
    var o = _rtRowToObj_(vals[i]);
    o.row_number = i + 2;
    rows.push(o);
  }
  return { ok:true, rows: rows };
}
function _rtNextFolio_(sh) {
  var year = new Date().getFullYear();
  var last = sh.getLastRow();
  var maxN = 0;
  if (last >= 2) {
    var folios = sh.getRange(2,2,last-1,1).getValues();
    for (var i=0;i<folios.length;i++) {
      var m = String(folios[i][0]||"").match(/^RT-(\d{4})-(\d+)$/);
  ...
  var next = maxN + 1;
  return "RT-" + year + "-" + ("000000" + next).slice(-6);
}
function rtUpsert_(data) {
  var sh = _rtEnsureSheet_();
  var p = data || {};
  var id = String(p.ID || p.id || "").trim();
  var now = new Date();
  var textFmt = []; for (var t=0;t<RT_HEADERS.length;t++) textFmt.push("@");
  var updatedBy = String(p.Updated_by || p.updated_by || "admin");
  if (id) {
    // Update
    var last = sh.getLastRow();
    if (last < 2) return { ok:false, error: "no encontrado" };
    ...
    for (var i=0;i<ids.length;i++) {
      if (String(ids[i][0]||"").trim() === id) {
        var row = i+2;
        var existing = sh.getRange(row,1,1,RT_HEADERS.length).getValues()[0];
        var merged = existing.slice();
        // Solo sobreescribir campos presentes en el payload
        for (var j=0;j<RT_HEADERS.length;j++) {
          var k = RT_HEADERS[j];
          if (p[k] !== undefined) merged[j] = _rtCoerce_(k, p[k]);
        }
        merged[33] = now; // Updated_at
        merged[34] = updatedBy; // Updated_by
        var range = sh.getRange(row,1,1,RT_HEADERS.length);
        range.setNumberFormats([textFmt]);
  ...
  // Insert
  id = "RT" + now.getTime() + "_" + Math.floor(Math.random()*10000);
  var folio = p.Folio || _rtNextFolio_(sh);
  var newRow = [];
  var payload = {
    ID: id,
    Folio: folio,
    ...
    Prioridad: p.Prioridad || "P3",
    Tipo: p.Tipo || "correctivo",
    ...
    Descripcion_solucion: p.Descripcion_solucion || "",
    Reportado_por: p.Reportado_por || "",
    Asignado_a: p.Asignado_a || "",
    Proveedor: p.Proveedor || "",
    Fecha_compromiso: p.Fecha_compromiso || "",
    Bloquea_habitabilidad: !!p.Bloquea_habitabilidad,
    Reincidente: !!p.Reincidente,
    Responsabilidad: p.Responsabilidad || "indeterminada",
    Costo_total: Number(p.Costo_total || 0),
    Cargar_a_huesped: !!p.Cargar_a_huesped,
    Motivo_espera: p.Motivo_espera || "",
    Motivo_cancelacion: p.Motivo_cancelacion || "",
    Motivo_reapertura: p.Motivo_reapertura || "",
    Reservacion_id: p.Reservacion_id || "",
    Huesped_nombre: p.Huesped_nombre || "",
    Huesped_contacto: p.Huesped_contacto || "",
    Fotos_antes_urls: (p.Fotos_antes_urls != null) ? String(p.Fotos_antes_urls) : "",
    Fotos_despues_urls: (p.Fotos_despues_urls != null) ? String(p.Fotos_despues_urls) : "",
    Notas: p.Notas || "",
    Updated_at: now,
    Updated_by: updatedBy,
  };
  for (var h=0;h<RT_HEADERS.length;h++) newRow.push(payload[RT_HEADERS[h]]);
  var target = sh.getRange(sh.getLastRow()+1, 1, 1, RT_HEADERS.length);
  target.setNumberFormats([textFmt]);
  target.setValues([newRow]);
  return { ok:true, id: id, folio: folio, created: true };
}
function _rtCoerce_(key, v) {
  if (key === "Bloquea_habitabilidad" || key === "Reincidente" || key === "Cargar_a_huesped") {
    return v === true || String(v).toUpperCase() === "TRUE";
  }
  if (key === "Costo_total") return Number(v) || 0;
  if (key === "Updated_at" || key === "Timestamp") return (v instanceof Date) ? v : new Date();
  return String(v == null ? "" : v);
}
function rtDelete_(data) {
  var id = String((data && (data.id || data.ID)) || "").trim();
  if (!id) return { ok:false, error: "id requerido" };
  var sh = _rtEnsureSheet_();
  var last = sh.getLastRow();
  if (last < 2) return { ok:false, error: "no encontrado" };
...
function rtUploadImage_(data) {
  // Reutiliza el uploader de incidencias para no duplicar lógica: mismo
  // patrón (base64 → Drive folder → URL pública).
  var p = data || {};
  if (!p.base64 || !p.mimeType) return { ok:false, error: "base64 + mimeType requeridos" };
  var folderName = "Reportes_Tecnicos";
  // Reutilizar helper de incidencias si existe.
  if (typeof _getOrCreateDriveFolder_ === "function" && typeof Utilities !== "undefined") {
    try {
      var folder = _getOrCreateDriveFolder_(folderName);
      var bytes = Utilities.base64Decode(p.base64);
      var blob = Utilities.newBlob(bytes, p.mimeType, p.name || ("rt_"+Date.now()+".jpg"));
      var file = folder.createFile(blob);
      file.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW);
      return { ok:true, url: "https://drive.google.com/uc?id=" + file.getId(), id: file.getId() };
    } catch(e) { return { ok:false, error: e.message }; }
  }
  return { ok:false, error: "helper de Drive no disponible" };
}

// ═══════════════════════════════════════════════════════════════════════════
// ║ LLAVES — control de llaves/códigos por alojamiento y persona           ║
// ║ Hoja "Llaves" con columnas:                                             ║
// ║   HouseId, Alojamiento, Puerta, Caja_seguridad, Claudia, Damariz,       ║
// ║   ACR, Oficina, Ultima_actualizacion, Verificado, Updated_by            ║
// ║ Endpoints: llaves_list (join con catálogo alojamientos), llaves_upsert  ║
// ═══════════════════════════════════════════════════════════════════════════

// Cada celda de las 6 columnas de tracking (Puerta, Caja_seguridad, Claudia,
// Damariz, Mantenimiento, Oficina) guarda 2 valores: STATE (''=default,
// 'V'=verificado, 'F'=falta) y DATE (fecha última vez que cambió).
var LLAVES_CELLS = ["Puerta","Caja_seguridad","Claudia","Damariz","Mantenimiento","Oficina"];
var LLAVES_HEADERS = ["HouseId","Alojamiento"]
  .concat(LLAVES_CELLS.reduce(function(acc, c){ return acc.concat([c + "_state", c + "_date"]); }, []))
  .concat(["Updated_by"]);
// Total: 2 + 6*2 + 1 = 15 columnas

function llavesEnsureSheet_() {
  var ss = getSpreadsheet_();
  var sh = ss.getSheetByName("Llaves");
  if (!sh) {
    sh = ss.insertSheet("Llaves");
    sh.getRange(1,1,1,LLAVES_HEADERS.length).setValues([LLAVES_HEADERS]);
    sh.setFrozenRows(1);
  } else {
    var row1 = sh.getRange(1,1,1,Math.max(LLAVES_HEADERS.length, sh.getLastColumn()||LLAVES_HEADERS.length)).getValues()[0];
    var needsWrite = false;
    for (var i=0; i<LLAVES_HEADERS.length; i++) {
      if ((row1[i] || "").toString().trim() !== LLAVES_HEADERS[i]) { needsWrite = true; break; }
    }
    if (needsWrite) sh.getRange(1,1,1,LLAVES_HEADERS.length).setValues([LLAVES_HEADERS]);
  }
  return sh;
}

/** Lista TODAS las filas de Llaves, joinéd con el catálogo alojamientos para
 *  garantizar que TODOS los alojamientos aparecen (aunque no tengan fila
 *  todavía en Llaves — se retornan con valores vacíos). Los alojamientos
 *  ocultos (Oculto = "Sí") se filtran fuera. */
function llavesList_() {
  var sh = llavesEnsureSheet_();
  var last = sh.getLastRow();
  var byId = {}; // HouseId → row values
  ...
  var normState = function(v) {
    var s = String(v||"").toUpperCase().trim();
    if (s === "V" || s === "F") return s;
    return "";
  };
  if (last >= 2) {
    var vals = sh.getRange(2, 1, last-1, LLAVES_HEADERS.length).getValues();
    for (var r=0; r<vals.length; r++) {
      var v = vals[r];
      var hid = String(v[0]||"").trim();
      if (!hid) continue;
      var item = {
        houseId: hid,
        alojamiento: String(v[1]||""),
        cells: {},
        updated_by: String(v[v.length-1]||""),
      };
      // Cada celda: 2 columnas consecutivas empezando en índice 2
      for (var k=0; k<LLAVES_CELLS.length; k++) {
        var stateCol = 2 + k*2;
        var dateCol  = stateCol + 1;
        item.cells[LLAVES_CELLS[k].toLowerCase()] = {
          state: normState(v[stateCol]),
          date:  fmtDate(v[dateCol]),
        };
      }
      byId[hid] = item;
    }
  }
  // Merge con catálogo alojamientos (fuente de verdad de qué alojamientos existen)
  var alSh = getSpreadsheet_().getSheetByName(ALOJAMIENTOS_SHEET);
  var items = [];
  if (alSh) {
    var alData = alSh.getDataRange().getValues();
    if (alData.length >= 2) {
      var alHead = alData[0].map(function(h){ return String(h||"").trim(); });
      var iHid  = alHead.indexOf("HouseId");
      var iProp = alHead.indexOf("Propiedad");
      var iDep  = alHead.indexOf("# Departamento");
      var iHid_alt = iHid;
      if (iHid < 0) iHid_alt = alHead.indexOf("houseId");
      var iOculto = alHead.indexOf("Oculto");
      var iDevice = alHead.indexOf("device_name");
      for (var i=1; i<alData.length; i++) {
        var arow = alData[i];
        var hid = String((iHid_alt>=0 ? arow[iHid_alt] : "") || "").trim();
        var prop = iProp>=0 ? String(arow[iProp]||"").trim() : "";
        var dep  = iDep >=0 ? String(arow[iDep] ||"").trim() : "";
        var device = iDevice>=0 ? String(arow[iDevice]||"").trim() : "";
        // Si no hay HouseId (alojamientos no-Lodgify como Plaza Versalles,
        // Calle Sevilla), usar una clave sintética: "dev:<device_name>" o
        // "pd:<propiedad>|<depto>". Así TODOS los alojamientos aparecen.
        if (!hid) {
          if (device) hid = "dev:" + device;
          else if (prop) hid = "pd:" + prop.toLowerCase() + "|" + dep;
          else continue; // realmente vacío
        }
        if (iOculto >= 0) {
          var oc = String(arow[iOculto]||"").toLowerCase().trim();
          if (oc === "sí" || oc === "si" || oc === "true" || oc === "1") continue;
        }
        var label = prop + (dep ? " #" + dep : "");
        var existing = byId[hid];
        if (existing) {
          if (!existing.alojamiento) existing.alojamiento = label;
          items.push(existing);
        } else {
          var emptyCells = {};
          for (var kk=0; kk<LLAVES_CELLS.length; kk++) {
            emptyCells[LLAVES_CELLS[kk].toLowerCase()] = { state: "", date: "" };
          }
          items.push({ houseId: hid, alojamiento: label, cells: emptyCells, updated_by: "" });
        }
      }
    }
  }
  items.sort(function(a,b){ return String(a.alojamiento||"").localeCompare(String(b.alojamiento||"")); });
  return { ok: true, items: items };
}

/** Actualiza UNA celda específica (o crea la fila si no existe).
 *  Payload esperado:
 *   { houseId, alojamiento, cell: 'puerta'|'caja_seguridad'|..., state: 'V'|'F'|'', date: 'YYYY-MM-DD' }
 *  Preserva las otras celdas de la misma fila (no las sobreescribe con vacío). */
function llavesUpsert_(data) {
  var houseId = String((data && data.houseId) || "").trim();
  if (!houseId) return { ok: false, error: "houseId requerido" };
  var cellKey = String((data && data.cell) || "").toLowerCase().trim();
  var validCells = LLAVES_CELLS.map(function(c){ return c.toLowerCase(); });
  if (cellKey && validCells.indexOf(cellKey) < 0) {
    return { ok: false, error: "cell inválida: " + cellKey };
  }
  var state = String((data && data.state) || "").toUpperCase();
  if (state !== "V" && state !== "F") state = "";
  var date = String((data && data.date) || "").slice(0,10);
  var alojamiento = String((data && data.alojamiento) || "");
  var upd = String((data && data.updated_by) || "admin");
  var sh = llavesEnsureSheet_();
  var last = sh.getLastRow();
  var rowIdx = -1;
  var currentRow = null;
  if (last >= 2) {
    var ids = sh.getRange(2, 1, last-1, 1).getValues();
    for (var i=0; i<ids.length; i++) {
      if (String(ids[i][0]||"").trim() === houseId) {
        rowIdx = i + 2;
        currentRow = sh.getRange(rowIdx, 1, 1, LLAVES_HEADERS.length).getValues()[0];
        break;
      }
    }
  }
  // Construir la fila nueva preservando lo existente y solo cambiando la celda especificada.
  var newRowData = [];
  newRowData.push(houseId);
  newRowData.push(alojamiento || (currentRow ? String(currentRow[1]||"") : ""));
  for (var k=0; k<LLAVES_CELLS.length; k++) {
    var thisCellKey = LLAVES_CELLS[k].toLowerCase();
    var stateColIdx = 2 + k*2;
    var dateColIdx  = stateColIdx + 1;
    if (cellKey === thisCellKey) {
      // Actualizar esta celda con los valores del payload
      newRowData.push(state);
      newRowData.push(date);
    } else {
      // Preservar valor existente (o vacío si no había fila)
      newRowData.push(currentRow ? String(currentRow[stateColIdx]||"") : "");
      var exDate = currentRow ? currentRow[dateColIdx] : "";
      if (exDate instanceof Date) {
        exDate = Utilities.formatDate(exDate, Session.getScriptTimeZone(), "yyyy-MM-dd");
      } else {
        exDate = String(exDate||"").slice(0,10);
      }
      newRowData.push(exDate);
    }
  }
  newRowData.push(upd);
  // Formato TEXT en TODAS las columnas para evitar que Sheets convierta "V" a fecha, etc.
  var textFmt = [new Array(LLAVES_HEADERS.length).fill("@")];
  if (rowIdx > 0) {
    var range = sh.getRange(rowIdx, 1, 1, LLAVES_HEADERS.length);
    range.setNumberFormats(textFmt);
    range.setValues([newRowData]);
    return { ok: true, houseId: houseId, updated: true };
  }
  var newRow = sh.getLastRow() + 1;
  var newRange = sh.getRange(newRow, 1, 1, LLAVES_HEADERS.length);
  newRange.setNumberFormats(textFmt);
...
// ═══════════════════════════════════════════════════════════════════════════
// ║ KPIs pre-computados por perfil (job diario)                              ║
// ║ Itera Reservas_Lodgify Status=Booked, agrupa por teléfono últimos 10    ║
// ║ dígitos, suma Nights + count(*) + sum(GrossTotal). Escribe en Perfiles. ║
// ║ Cloud Scheduler dispara POST /perfiles-recalc-kpis diariamente.         ║
// ═══════════════════════════════════════════════════════════════════════════
function perfilesRecalcKpis_(data) {
  ...
  var pfVals = pfRange.getValues();
  var nowIso = new Date().toISOString();
  var updated = 0;
  for (var j = 0; j < pfVals.length; j++) {
  ...
  return {
    ok: true,
    updated: updated,
    total_perfiles: pfVals.length,
    phones_con_bookings: Object.keys(kpisByPhone).length,
    elapsed_ms: Date.now() - startMs,
    last_run: nowIso,
  };
}
...
function perfilesKpisList_() {
  var pfSh = getSheet_(PERFILES_SHEET);
  var pfHeaders = getHeaders_(pfSh);
  var iPhone = pfHeaders.indexOf("Cel/Whatsapp (principal)");
  ...
  var byPhone = {};
  if (pfLast >= 2) {
    var vals = pfSh.getRange(2, 1, pfLast - 1, pfHeaders.length).getValues();
    for (var i = 0; i < vals.length; i++) {
  ...
  return { ok: true, by_phone: byPhone, total: Object.keys(byPhone).length };
}

// ═══════════════════════════════════════════════════════════════════════════
// ║ bookings_by_guest — devuelve el historial COMPLETO de bookings para    ║
// ║ un teléfono. Usado en el on-demand al expandir card en Gestión reservas║
// ║ Params: phone (10+ dígitos, se usan últimos 10)                         ║
// ═══════════════════════════════════════════════════════════════════════════
function bookingsByGuest_(data) {
  var phRaw = String((data && data.phone) || "").replace(/\D/g, "");
  if (phRaw.length < 10) return { ok: false, error: "phone requerido (10 dígitos)" };
  var phKey = phRaw.slice(-10);
  var ss = getSpreadsheet_();
  var bookings = [];
  var lgSh = ss.getSheetByName(LODGIFY_SHEET);
  if (lgSh) {
    ...
    if (lgLast >= 2) {
      ...
      if (iPhone >= 0) {
        var lgVals = lgSh.getRange(2, 1, lgLast - 1, lgHeaders.length).getValues();
        for (var i = 0; i < lgVals.length; i++) {
          var phRow = String(lgVals[i][iPhone] || "").replace(/\D/g, "");
          if (phRow.length < 10) continue;
          if (phRow.slice(-10) !== phKey) continue;
          var obj = {};
          for (var c = 0; c < lgHeaders.length; c++) {
            obj[lgHeaders[c]] = lgVals[i][c];
          }
          bookings.push(obj);
        }
      }
  ...
  // Rows de la hoja Reservaciones (manuales o propagadas). Front las usa en
  // _waFindRelatedBookings para armar accordions sin necesidad de bajar toda
  // la tabla (huespedesLoad) al abrir el modal WA.
  var huRows = [];
  try {
    var huSh = ss.getSheetByName(RESERVACIONES_SHEET);
    if (huSh) {
      var huLast = huSh.getLastRow();
      if (huLast >= 2) {
        var huHeaders = huSh.getRange(1, 1, 1, huSh.getLastColumn()).getValues()[0];
        var iHuPh = huHeaders.indexOf("Cel/Whatsapp (principal)");
        if (iHuPh >= 0) {
          var huVals = huSh.getRange(2, 1, huLast - 1, huHeaders.length).getValues();
          for (var j = 0; j < huVals.length; j++) {
            var huPh = String(huVals[j][iHuPh] || "").replace(/\D/g, "");
            if (huPh.length < 10 || huPh.slice(-10) !== phKey) continue;
            var rObj = {};
            for (var k = 0; k < huHeaders.length; k++) rObj[huHeaders[k]] = huVals[j][k];
            rObj.row_number = j + 2;
            huRows.push(rObj);
          }
        }
  ...
  return { ok: true, bookings: bookings, count: bookings.length, huRows: huRows };
}

// ═══════════════════════════════════════════════════════════════════════════
// ║ BOT IA WhatsApp — Contexto de conversación + control admin              ║
// ║                                                                          ║
// ║ Hoja WA_ChatContext: 1 fila por mensaje (histórico completo).           ║
// ║   phone10 | timestamp | role (user|assistant|admin) | body | meta_json  ║
// ║                                                                          ║
// ║ Hoja WA_ChatState: 1 fila por conversación (control).                   ║
// ║   phone10 | control (bot|human) | last_msg_at | last_msg_preview |      ║
// ║   escalated_at | escalated_reason | notes                                ║
// ═══════════════════════════════════════════════════════════════════════════
var WA_CHATCONTEXT_SHEET = "WA_ChatContext";
var WA_CHATCONTEXT_HEADERS = ["phone10","timestamp","role","body","meta_json"];
var WA_CHATSTATE_SHEET = "WA_ChatState";
var WA_CHATSTATE_HEADERS = ["phone10","control","last_msg_at","last_msg_preview","escalated_at","escalated_reason","notes","pending_draft_body","pending_draft_at"];
var WA_CHAT_CONTEXT_MAX_ROWS = 20000; // rolling window

function _waChatEnsureSheets_() {
  var ss = getSpreadsheet_();
  var shC = ss.getSheetByName(WA_CHATCONTEXT_SHEET);
  if (!shC) {
    shC = ss.insertSheet(WA_CHATCONTEXT_SHEET);
    shC.getRange(1, 1, 1, WA_CHATCONTEXT_HEADERS.length).setValues([WA_CHATCONTEXT_HEADERS]).setFontWeight("bold").setBackground("#f1f5f9");
    shC.setFrozenRows(1);
  }
  var shS = ss.getSheetByName(WA_CHATSTATE_SHEET);
  if (!shS) {
    shS = ss.insertSheet(WA_CHATSTATE_SHEET);
    shS.getRange(1, 1, 1, WA_CHATSTATE_HEADERS.length).setValues([WA_CHATSTATE_HEADERS]).setFontWeight("bold").setBackground("#f1f5f9");
    shS.setFrozenRows(1);
  } else {
    // Migración: añadir columnas pending_draft_* si faltan.
    var lastCol = shS.getLastColumn();
    var cur = shS.getRange(1, 1, 1, lastCol).getValues()[0];
    var needAdd = WA_CHATSTATE_HEADERS.length - cur.length;
    if (needAdd > 0) {
      var extras = WA_CHATSTATE_HEADERS.slice(cur.length);
      shS.getRange(1, cur.length + 1, 1, extras.length).setValues([extras]).setFontWeight("bold").setBackground("#f1f5f9");
    }
  }
  return { context: shC, state: shS };
}
function _waNormalizePhone10_(v) {
  var s = String(v || "").replace(/\D/g, "");
  return s.length >= 10 ? s.slice(-10) : "";
}

/** GET últimos N msgs del historial + estado de control. */
function waChatContextGet_(data) {
  var phone = _waNormalizePhone10_(data && (data.phone || data.phone10));
  if (!phone) return { ok: false, error: "phone requerido" };
  var limit = Math.min(parseInt((data && data.limit) || 20, 10) || 20, 200);
  var sheets = _waChatEnsureSheets_();
  var shC = sheets.context;
  var lastRow = shC.getLastRow();
  var msgs = [];
  if (lastRow >= 2) {
    var vals = shC.getRange(2, 1, lastRow - 1, WA_CHATCONTEXT_HEADERS.length).getValues();
    for (var i = 0; i < vals.length; i++) {
      if (String(vals[i][0]) !== phone) continue;
      msgs.push({
        timestamp: vals[i][1] instanceof Date ? vals[i][1].toISOString() : String(vals[i][1] || ""),
        role: String(vals[i][2] || ""),
        body: String(vals[i][3] || ""),
        meta: vals[i][4] ? (function(x){ try { return JSON.parse(x); } catch(_) { return {}; }})(vals[i][4]) : {}
      });
    }
  }
  msgs = msgs.slice(-limit);
  // Estado
  var shS = sheets.state;
  var stateLast = shS.getLastRow();
  var state = { control: "bot", last_msg_at: "", escalated_reason: "" };
  if (stateLast >= 2) {
    var svals = shS.getRange(2, 1, stateLast - 1, WA_CHATSTATE_HEADERS.length).getValues();
    for (var j = 0; j < svals.length; j++) {
      if (String(svals[j][0]) === phone) {
        state = {
          control: String(svals[j][1] || "bot"),
          last_msg_at: svals[j][2] instanceof Date ? svals[j][2].toISOString() : String(svals[j][2] || ""),
          escalated_at: svals[j][4] instanceof Date ? svals[j][4].toISOString() : String(svals[j][4] || ""),
          escalated_reason: String(svals[j][5] || ""),
          pending_draft_body: String(svals[j][7] || ""),
          pending_draft_at: svals[j][8] instanceof Date ? svals[j][8].toISOString() : String(svals[j][8] || ""),
        };
        break;
      }
    }
  }
  return { ok: true, phone: phone, messages: msgs, state: state };
}

/** APPEND un mensaje al historial + actualiza state. */
function waChatContextAppend_(data) {
  var phone = _waNormalizePhone10_(data && (data.phone || data.phone10));
  if (!phone) return { ok: false, error: "phone requerido" };
  var role = String((data && data.role) || "").toLowerCase();
  if (!/^(user|assistant|admin|system|template)$/.test(role)) return { ok: false, error: "role invalido" };
  var body = String((data && data.body) || "").slice(0, 4000);
  var meta = (data && data.meta) || {};
  var sheets = _waChatEnsureSheets_();
  var shC = sheets.context;
  var nowIso = new Date().toISOString();
  shC.appendRow([phone, nowIso, role, body, JSON.stringify(meta)]);
  // Rolling cap
  var lastRow = shC.getLastRow();
  if (lastRow - 1 > WA_CHAT_CONTEXT_MAX_ROWS) {
    var excess = lastRow - 1 - WA_CHAT_CONTEXT_MAX_ROWS;
    shC.deleteRows(2, excess);
  }
  // State upsert (preview del último msg)
  var shS = sheets.state;
  var stateLast = shS.getLastRow();
  ...
  if (stateLast >= 2) {
    ...
    for (var i = 0; i < svals.length; i++) {
      if (String(svals[i][0]) === phone) {
        // Actualizar last_msg_at + preview, conservar control/escalated
        shS.getRange(i + 2, 3).setValue(nowIso);
        shS.getRange(i + 2, 4).setValue(preview);
        return { ok: true, appended: true, phone: phone };
      }
    }
  }
  shS.appendRow([phone, "bot", nowIso, preview, "", "", ""]);
  return { ok: true, appended: true, phone: phone, created_state: true };
}

/** SET control (bot|human) o marca escalado. */
function waChatSetControl_(data) {
  var phone = _waNormalizePhone10_(data && (data.phone || data.phone10));
  if (!phone) return { ok: false, error: "phone requerido" };
  var control = String((data && data.control) || "").toLowerCase();
  if (!/^(bot|human|supervised)$/.test(control)) return { ok: false, error: "control invalido (bot|human|supervised)" };
  ...
  if (stateLast >= 2) {
    ...
    for (var i = 0; i < svals.length; i++) {
      if (String(svals[i][0]) === phone) {
        shS.getRange(i + 2, 2).setValue(control);
        if (control === "human") {
          shS.getRange(i + 2, 5).setValue(nowIso);
          if (reason) shS.getRange(i + 2, 6).setValue(reason);
        }
        if (notes) shS.getRange(i + 2, 7).setValue(notes);
        return { ok: true, phone: phone, control: control };
      }
    }
  }
  shS.appendRow([phone, control, "", "", control === "human" ? nowIso : "", reason, notes, "", ""]);
  return { ok: true, phone: phone, control: control, created: true };
}

/** Upsert pending draft body/at para modo supervised. body="" limpia. */
function waChatSetDraft_(data) {
  var phone = _waNormalizePhone10_(data && (data.phone || data.phone10));
  if (!phone) return { ok: false, error: "phone requerido" };
  ...
  var shS = sheets.state;
  var stateLast = shS.getLastRow();
  var nowIso = body ? new Date().toISOString() : "";
  if (stateLast >= 2) {
    var svals = shS.getRange(2, 1, stateLast - 1, WA_CHATSTATE_HEADERS.length).getValues();
    for (var i = 0; i < svals.length; i++) {
      if (String(svals[i][0]) === phone) {
        shS.getRange(i + 2, 8).setValue(body);
        shS.getRange(i + 2, 9).setValue(nowIso);
        return { ok: true, phone: phone };
      }
    }
  }
  shS.appendRow([phone, "supervised", "", "", "", "", "", body, nowIso]);
  return { ok: true, phone: phone, created: true };
}

/** LIST conversaciones activas (para panel admin). Incluye nombre del perfil
 *  (Nombre del huésped) matcheando el phone10 contra la hoja Perfiles. */
function waChatConversationsList_(data) {
  var limit = Math.min(parseInt((data && data.limit) || 100, 10) || 100, 500);
  var filter = String((data && data.filter) || "all").toLowerCase();
  var sheets = _waChatEnsureSheets_();
  var shS = sheets.state;
  var stateLast = shS.getLastRow();
  if (stateLast < 2) return { ok: true, conversations: [] };
  // Precargar mapa phone10 → { nombre, razon_social, regimen_fiscal } desde Perfiles.
  var profileByPhone = {};
  try {
    var pfSh = getSheet_(PERFILES_SHEET);
    var pfHeaders = getHeaders_(pfSh);
    var iPh  = pfHeaders.indexOf("Cel/Whatsapp (principal)");
    var iNom = pfHeaders.indexOf("Nombre del huésped");
    var iRS  = pfHeaders.indexOf("Razón social");
    var iReg = pfHeaders.indexOf("Régimen fiscal");
    if (iPh >= 0 && iNom >= 0) {
      var pfLast = pfSh.getLastRow();
      if (pfLast >= 2) {
        var pfVals = pfSh.getRange(2, 1, pfLast - 1, pfHeaders.length).getValues();
        for (var k = 0; k < pfVals.length; k++) {
          var raw = String(pfVals[k][iPh] || "").replace(/\D/g,"");
          if (raw.length >= 10) {
            profileByPhone[raw.slice(-10)] = {
              nombre:        String(pfVals[k][iNom] || "").trim(),
              razon_social:  iRS  >= 0 ? String(pfVals[k][iRS] || "").trim()  : "",
              regimen_fiscal: iReg >= 0 ? String(pfVals[k][iReg] || "").trim() : "",
            };
          }
        }
      }
  ...
  var nameByPhone = {};
  Object.keys(profileByPhone).forEach(function(p){ nameByPhone[p] = profileByPhone[p].nombre; });
  var svals = shS.getRange(2, 1, stateLast - 1, WA_CHATSTATE_HEADERS.length).getValues();
  // Pre-cómputo del rol del ÚLTIMO mensaje por phone (para saber si está
  // "sin responder" — cuando el último es 'user').
  var lastRoleByPhone = {};
  try {
    var shC = sheets.context;
    var cLast = shC.getLastRow();
    if (cLast >= 2) {
      var cVals = shC.getRange(2, 1, cLast - 1, 3).getValues(); // phone10, timestamp, role
      var lastTsByPhone = {};
      for (var m = 0; m < cVals.length; m++) {
        var ph = String(cVals[m][0] || "");
        if (!ph) continue;
        var ts = cVals[m][1] instanceof Date ? cVals[m][1].getTime() : Date.parse(String(cVals[m][1] || ""));
        if (isNaN(ts)) ts = 0;
        var rl = String(cVals[m][2] || "");
        if (!lastTsByPhone[ph] || ts >= lastTsByPhone[ph]) {
          lastTsByPhone[ph] = ts;
          lastRoleByPhone[ph] = rl;
        }
      }
    }
  } catch (e) {}
  var convs = [];
  for (var i = 0; i < svals.length; i++) {
    var control = String(svals[i][1] || "bot");
    if (filter === "bot" && control !== "bot") continue;
    if (filter === "human" && control !== "human") continue;
    if (filter === "supervised" && control !== "supervised") continue;
    if (filter === "pending" && control !== "human") continue;
    var phone10 = String(svals[i][0] || "");
    convs.push({
      phone: phone10,
      name: nameByPhone[phone10] || "",
      razon_social:  (profileByPhone[phone10] && profileByPhone[phone10].razon_social) || "",
      regimen_fiscal:(profileByPhone[phone10] && profileByPhone[phone10].regimen_fiscal) || "",
      control: control,
      last_msg_at: svals[i][2] instanceof Date ? svals[i][2].toISOString() : String(svals[i][2] || ""),
      last_msg_preview: String(svals[i][3] || ""),
      escalated_at: svals[i][4] instanceof Date ? svals[i][4].toISOString() : String(svals[i][4] || ""),
      escalated_reason: String(svals[i][5] || ""),
      notes: String(svals[i][6] || ""),
      pending_draft_body: String(svals[i][7] || ""),
      pending_draft_at: svals[i][8] instanceof Date ? svals[i][8].toISOString() : String(svals[i][8] || ""),
      last_msg_role: lastRoleByPhone[phone10] || "",
    });
  }
  convs.sort(function(a, b) { return String(b.last_msg_at).localeCompare(String(a.last_msg_at)); });
  return { ok: true, conversations: convs.slice(0, limit), total: convs.length };
}

/** LIST TODOS los mensajes WhatsApp asociados a un teléfono — timeline unificado:
 *  - Bot conversation (WA_ChatContext): user, assistant, admin
 *  - Envíos programados (WA_Logs): auto templates, tickets, recordatorios
 *  Devuelve ordenado por timestamp asc. */

/** Registra una solicitud de late checkout en la hoja Reservaciones.
 *  Payload: { phone, arrival?, hora_nueva }.
 *  Match: (phone + arrival) preferido; si arrival no viene, la reserva
 *  cuyo Fecha de ingreso sea más cercana a hoy. La columna "LateCheckout"
 *  se crea automáticamente si no existe. Formato del valor:
 *  "HH:MM · pendiente · YYYY-MM-DD HH:mm".
 *  El chip visual en el admin (Gestión de reservas + Chats bot) lee este
 *  campo y muestra "🕐 Late HH:MM" con color según estado. */
function waSetLateCheckout_(data) {
  var p = data || {};
  var phone = _waNormalizePhone10_(String(p.phone || p.telefono || ""));
  var arrival = String(p.arrival || "").slice(0, 10);
  var hora = String(p.hora_nueva || p.hora || "").trim();
  if (!phone) return { ok: false, error: "phone requerido" };
  if (!/^\d{1,2}:\d{2}$/.test(hora)) return { ok: false, error: "hora_nueva inválida (HH:MM)" };
  var sh = _rvEnsureSheet_ ? _rvEnsureSheet_() : SpreadsheetApp.getActive().getSheetByName(RESERVACIONES_SHEET);
  if (!sh) return { ok: false, error: "Hoja Reservaciones no encontrada" };
  var lastRow = sh.getLastRow(); var lastCol = sh.getLastColumn();
  if (lastRow < 2) return { ok: false, error: "Reservaciones vacía" };
  var headers = sh.getRange(1, 1, 1, lastCol).getValues()[0];
  var iPhone = headers.indexOf("Celular");
  if (iPhone < 0) iPhone = headers.indexOf("Teléfono");
  var iArr = headers.indexOf("Fecha de ingreso");
  var iLate = headers.indexOf("LateCheckout");
  if (iLate < 0) {
    sh.getRange(1, lastCol + 1).setValue("LateCheckout");
    iLate = lastCol; lastCol++;
  }
  var vals = sh.getRange(2, 1, lastRow - 1, lastCol).getValues();
  var candidates = [];
  var todayIso = Utilities.formatDate(new Date(), "America/Monterrey", "yyyy-MM-dd");
  for (var i = 0; i < vals.length; i++) {
    var rp = _waNormalizePhone10_(String(vals[i][iPhone] || ""));
    if (rp !== phone) continue;
    var rArr = iArr >= 0 ? (vals[i][iArr] instanceof Date
      ? Utilities.formatDate(vals[i][iArr], "America/Monterrey", "yyyy-MM-dd")
      : String(vals[i][iArr] || "").slice(0, 10)) : "";
    if (arrival && rArr && rArr !== arrival) continue;
    candidates.push({ row: i + 2, arr: rArr });
  }
  if (!candidates.length) return { ok: false, error: "reservación no encontrada" };
  // Escoger la reserva con arrival ≥ hoy más cercana; si no hay futura, la más reciente.
  candidates.sort(function(a, b) {
    var af = a.arr >= todayIso, bf = b.arr >= todayIso;
    if (af !== bf) return af ? -1 : 1;
    return af ? a.arr.localeCompare(b.arr) : b.arr.localeCompare(a.arr);
  });
  var target = candidates[0];
  var stamp = Utilities.formatDate(new Date(), "America/Monterrey", "yyyy-MM-dd HH:mm");
  sh.getRange(target.row, iLate + 1).setValue(hora + " · pendiente · " + stamp);
  return { ok: true, row: target.row, hora: hora, arrival: target.arr };
}

function waAllMessagesByPhone_(data) {
  var phone = _waNormalizePhone10_(data && (data.phone || data.phone10));
  if (!phone) return { ok: false, error: "phone requerido" };
  var limit = Math.min(parseInt((data && data.limit) || 500, 10) || 500, 2000);
  var messages = [];
  // 1) WA_ChatContext (bot user/assistant/admin)
  try {
    var shC = _waChatEnsureSheets_().context;
    var lastRow = shC.getLastRow();
    if (lastRow >= 2) {
      var vals = shC.getRange(2, 1, lastRow - 1, WA_CHATCONTEXT_HEADERS.length).getValues();
      for (var i = 0; i < vals.length; i++) {
  ...
  // 2) WA_Logs (envíos programados / cron / manual admin desde módulo WA)
  try {
    var ss = getSpreadsheet_();
    var shL = ss.getSheetByName("WA_Logs");
    if (shL) {
      var lgLast = shL.getLastRow();
      if (lgLast >= 2) {
        var lgHeaders = shL.getRange(1, 1, 1, shL.getLastColumn()).getValues()[0].map(function(v){ return String(v||"").trim(); });
        var iTo = lgHeaders.indexOf("to");
        var iTs = lgHeaders.indexOf("timestamp");
        var iBody = lgHeaders.indexOf("body_preview");
        var iTipo = lgHeaders.indexOf("tipo");
        var iStatus = lgHeaders.indexOf("status");
        if (iTo >= 0 && iTs >= 0 && iBody >= 0) {
          var lgVals = shL.getRange(2, 1, lgLast - 1, lgHeaders.length).getValues();
          for (var j = 0; j < lgVals.length; j++) {
            var toRaw = String(lgVals[j][iTo] || "").replace(/\D/g,"");
            if (toRaw.slice(-10) !== phone) continue;
            messages.push({
              timestamp: lgVals[j][iTs] instanceof Date ? lgVals[j][iTs].toISOString() : String(lgVals[j][iTs] || ""),
              role: "outbound",
              body: String(lgVals[j][iBody] || ""),
              source: "wa-log",
              tipo: iTipo >= 0 ? String(lgVals[j][iTipo] || "") : "",
              status: iStatus >= 0 ? String(lgVals[j][iStatus] || "") : "",
            });
          }
        }
      }
  ...
  // Sort asc
  messages.sort(function(a, b) { return String(a.timestamp).localeCompare(String(b.timestamp)); });
  return { ok: true, phone: phone, messages: messages.slice(-limit), count: messages.length };
}

/** SET bot_enabled masivo: recibe { houseIds: [...] } y pone TRUE para esos,
 *  FALSE (vacío) para el resto. Marca cambios en la hoja alojamientos. */
function waBotAlojamientosSet_(data) {
  var wanted = new Set();
  if (data && Array.isArray(data.houseIds)) {
    data.houseIds.forEach(function(h){ wanted.add(String(h).trim()); });
  }
  var sh = getSpreadsheet_().getSheetByName("alojamientos");
  if (!sh) return { ok: false, error: "hoja alojamientos no encontrada" };
  var headers = sh.getRange(1, 1, 1, sh.getLastColumn()).getValues()[0].map(function(v){ return String(v||"").trim(); });
  var iHouseId = headers.indexOf("HouseId");
  var iIdLodgify = headers.indexOf("id_lodgify");
  var iBotEnabled = headers.indexOf("bot_enabled");
  if (iBotEnabled < 0) {
    sh.getRange(1, headers.length + 1).setValue("bot_enabled");
    iBotEnabled = headers.length;
  }
  var lastRow = sh.getLastRow();
  if (lastRow < 2) return { ok: true, updated: 0 };
  var range = sh.getRange(2, iBotEnabled + 1, lastRow - 1, 1);
  var idsHid = iHouseId >= 0 ? sh.getRange(2, iHouseId + 1, lastRow - 1, 1).getValues() : [];
  var idsLg  = iIdLodgify >= 0 ? sh.getRange(2, iIdLodgify + 1, lastRow - 1, 1).getValues() : [];
  var current = range.getValues();
  var updated = 0;
  for (var i = 0; i < current.length; i++) {
    // Fallback HouseId → id_lodgify (misma razón que en waBotAlojamientosList_).
    var h = String((idsHid[i] && idsHid[i][0]) || "").trim();
    if (!h) h = String((idsLg[i] && idsLg[i][0]) || "").trim();
    var shouldEnable = h && wanted.has(h);
    var currentVal = String(current[i][0] || "").toUpperCase();
    var isEnabled = (currentVal === "TRUE" || currentVal === "1");
    if (shouldEnable !== isEnabled) {
      current[i][0] = shouldEnable ? true : "";
      updated++;
    }
  }
  range.setValues(current);
  return { ok: true, updated: updated, total_enabled: wanted.size };
}

/** LIST alojamientos con flag bot_enabled (para piloto restringido). */
function waBotAlojamientosList_() {
  var sh = getSpreadsheet_().getSheetByName("alojamientos");
  if (!sh) return { ok: false, error: "hoja alojamientos no encontrada" };
  var headers = sh.getRange(1, 1, 1, sh.getLastColumn()).getValues()[0].map(function(v){ return String(v||"").trim(); });
  var iHouseId = headers.indexOf("HouseId");
  ...
  if (iBotEnabled < 0) {
    iBotEnabled = headers.length;
    sh.getRange(1, iBotEnabled + 1).setValue("bot_enabled");
  }
  var lastRow = sh.getLastRow();
  var result = [];
  if (lastRow >= 2) {
    var vals = sh.getRange(2, 1, lastRow - 1, headers.length + (iBotEnabled >= headers.length ? 1 : 0)).getValues();
    for (var i = 0; i < vals.length; i++) {
      // FIX: la columna HouseId suele estar vacía; el ID real vive en id_lodgify.
      // Sin este fallback, waBotAlojamientosList_ devolvía [] y el bot rechazaba
      // TODOS los mensajes en modo Automático con "aloj X no en piloto".
      var h = (iHouseId >= 0 ? String(vals[i][iHouseId] || "").trim() : "");
      if (!h && iIdLodgify >= 0) h = String(vals[i][iIdLodgify] || "").trim();
      if (!h) continue;
      var enabledRaw = vals[i][iBotEnabled];
      var enabled = enabledRaw === true || String(enabledRaw).toUpperCase() === "TRUE" || String(enabledRaw).trim() === "1";
      result.push({
        HouseId: h,
        Propiedad: String(vals[i][headers.indexOf("Propiedad")] || ""),
        Departamento: String(vals[i][headers.indexOf("# Departamento")] || ""),
        bot_enabled: enabled,
      });
    }
  }
  return { ok: true, alojamientos: result };
}
...
// ═══════════════════════════════════════════════════════════════════════════
// ║ Perfil por teléfono (get + upsert) — usado por Chats bot → Editar perfil.║
// ═══════════════════════════════════════════════════════════════════════════
function _normalizePhone10_(v) {
  var s = String(v || "").replace(/\D/g, "");
  return s.length >= 10 ? s.slice(-10) : s;
}
function perfilGetByPhone_(data) {
  ...
  var iPh = headers.indexOf("Cel/Whatsapp (principal)");
  if (iPh < 0) return { ok:false, error:"columna Cel/Whatsapp (principal) no existe en Perfiles" };
  var last = sh.getLastRow();
  if (last < 2) return { ok:true, perfil:null, phone:p10 };
  var vals = sh.getRange(2, 1, last-1, headers.length).getValues();
  for (var i = 0; i < vals.length; i++) {
    var raw = String(vals[i][iPh] || "").replace(/\D/g,"");
    if (raw.length >= 10 && raw.slice(-10) === p10) {
      var perfil = {};
      for (var c = 0; c < headers.length; c++) perfil[headers[c]] = vals[i][c];
      perfil._row = i + 2;
      return { ok:true, perfil:perfil, phone:p10 };
    }
  }
  return { ok:true, perfil:null, phone:p10 };
}
/**
 * Normaliza un código de confirmación (Airbnb tipo "HMC9QRDTWR" o Lodgify tipo
 * "#B23006918" o "B23006918"). Uppercase, trim, quita espacios internos.
 * El "#" inicial es opcional para el match — ambas variantes se aceptan.
 */
function _normalizeConfirmationCode_(v) {
  // Acepta: "HMC9QRDTWR" (Airbnb), "#B23006918" / "B23006918" / "#23006918" /
  // "23006918" (Lodgify). Quita "#", "B" inicial de Lodgify, y espacios.
  var s = String(v || "").toUpperCase().replace(/\s+/g, "").replace(/^#/, "").trim();
  // Si empieza con "B" y todo lo demás es dígito → prefijo Lodgify → quitarlo.
  if (/^B\d+$/.test(s)) s = s.slice(1);
  return s;
}

/**
 * Busca en Reservaciones por "Código de confirmación". Si encuentra, devuelve
 * la reserva y el perfil asociado (via Cel/Whatsapp (principal)). Si la reserva
 * existe pero el perfil no, devuelve solo la reserva. Si no se encuentra, ok:true
 * con perfil:null y reserva:null.
 */
function reservaGetByConfirmationCode_(data) {
  ...
  var iPh   = headers.indexOf("Cel/Whatsapp (principal)");
  if (iCode < 0) return { ok:false, error:"columna 'Código de confirmación' no existe en Reservaciones" };
  var last = sh.getLastRow();
  if (last < 2) return { ok:true, reserva:null, perfil:null, code:code };
  var vals = sh.getRange(2, 1, last-1, headers.length).getValues();
  // Escaneo desde el final para preferir la coincidencia más reciente
  for (var i = vals.length - 1; i >= 0; i--) {
    var raw = _normalizeConfirmationCode_(vals[i][iCode]);
    if (raw && raw === code) {
      var reserva = {};
  ...
  // Airbnb "HM..." vive ahí incluso si el huésped todavía no llenó el check-in.
  var dbg = { lg_sheet_exists:false, lg_rows:0, lg_iCC:-1, lg_iCB:-1, lg_cc_nonempty:0, lg_last5:[] };
  try {
    var ss = getSpreadsheet_();
    var shL = ss.getSheetByName(LODGIFY_SHEET);
    if (shL) dbg.lg_sheet_exists = true;
    if (shL && shL.getLastRow() >= 2) {
      var lgHeaders = getHeaders_(shL);
      var iCC = lgHeaders.indexOf("ConfirmationCode");
      var iCB = lgHeaders.indexOf("ChannelBooking");
      var iBId = lgHeaders.indexOf("Id");
      dbg.lg_iCC = iCC; dbg.lg_iCB = iCB;
      dbg.lg_rows = shL.getLastRow() - 1;
      ...
      if (iCC >= 0 || iCB >= 0) {
        ...
        if (matchIdx >= 0) {
          ...
          var j = 0;
          {
            ...
            var perfil2 = null;
            if (phoneL) {
              var pr2 = perfilGetByPhone_({ phone: phoneL });
              if (pr2 && pr2.ok && pr2.perfil) perfil2 = pr2.perfil;
            }
            // Resuelve Propiedad + # Departamento desde alojamientos usando
            // HouseId / id_lodgify / RoomTypeIds (HouseName suele venir vacío
            // desde Lodgify → memoria: project_house_id_none).
            var propResuelta = "", deptoResuelto = "";
            try {
              var iRHId = lgHeaders.indexOf("HouseId");
              var iRTIds = lgHeaders.indexOf("RoomTypeIds");
              var lgHouseId = iRHId >= 0 ? String(lgVals[j][iRHId] || "").trim() : "";
              var lgRTIds   = iRTIds >= 0 ? String(lgVals[j][iRTIds] || "").trim() : "";
              var alSh = ss.getSheetByName(ALOJAMIENTOS_SHEET);
              if (alSh && alSh.getLastRow() >= 2) {
                var alHdr = alSh.getRange(1, 1, 1, alSh.getLastColumn()).getValues()[0]
                  .map(function(h){ return String(h||"").trim(); });
                var aiHId  = alHdr.indexOf("HouseId");
                var aiIdLg = alHdr.indexOf("id_lodgify");
                var aiRTId = alHdr.indexOf("RoomTypeIds");
                var aiProp = alHdr.indexOf("Propiedad");
                var aiDpto = alHdr.indexOf("# Departamento");
                if (aiProp >= 0 && aiDpto >= 0) {
                  var alVals = alSh.getRange(2, 1, alSh.getLastRow()-1, alHdr.length).getValues();
                  for (var kk = 0; kk < alVals.length; kk++) {
                    var hh = aiHId  >= 0 ? String(alVals[kk][aiHId]  || "").trim() : "";
                    var il = aiIdLg >= 0 ? String(alVals[kk][aiIdLg] || "").trim() : "";
                    var rt = aiRTId >= 0 ? String(alVals[kk][aiRTId] || "").trim() : "";
                    var matches = false;
                    if (lgHouseId && (hh === lgHouseId || il === lgHouseId)) matches = true;
                    else if (lgRTIds && rt === lgRTIds) matches = true;
                    if (matches) {
                      propResuelta = String(alVals[kk][aiProp] || "").trim();
                      deptoResuelto = String(alVals[kk][aiDpto] || "").trim();
                      break;
                    }
                  }
                }
            ...
            var reservaMapped = {
              ...
              "Fecha de salida":   iDD >= 0 ? String(lgVals[j][iDD] || "") : "",
              "# Noches":          iNg >= 0 ? lgVals[j][iNg] : "",
              "# Huéspedes":       iN  >= 0 ? lgVals[j][iN]  : "",
    ...
    Logger.log("[reservaGetByConfirmationCode_ lodgify-fallback] " + e);
    dbg.err = String(e);
  }
  return { ok:true, reserva:null, perfil:null, code:code, debug:dbg };
}

/**
 * Devuelve las reservas de Lodgify asociadas a un teléfono que están activas
 * o próximas (DateDeparture >= hoy) y no canceladas/eliminadas. Ordenadas por
 * DateArrival ascendente. Cada item lleva los campos suficientes para prefill
 * y para render en el selector.
 */
function reservasByPhone_(data) {
  ...
  var cacheKey = "rbp_v9_registrado_" + p10;
  try {
  ...
  var iPh = hdr.indexOf("GuestPhone");
  var iSt = hdr.indexOf("Status");
  var iDA = hdr.indexOf("DateArrival");
  var iDD = hdr.indexOf("DateDeparture");
  var iNg = hdr.indexOf("Nights");
  var iNu = hdr.indexOf("NumberOfGuests");
  var iHN = hdr.indexOf("HouseName");
  var iHId = hdr.indexOf("HouseId");
  var iRTIds = hdr.indexOf("RoomTypeIds");
  var iCC = hdr.indexOf("ConfirmationCode");
  var iCB = hdr.indexOf("ChannelBooking");
  ...
  var alRows = [], alHdr = [];
  if (alSh && alSh.getLastRow() >= 2) {
    alHdr = alSh.getRange(1, 1, 1, alSh.getLastColumn()).getValues()[0]
      .map(function(h){ return String(h||"").trim(); });
    alRows = alSh.getRange(2, 1, alSh.getLastRow()-1, alHdr.length).getValues();
  }
  var aiHId = alHdr.indexOf("HouseId"), aiIdLg = alHdr.indexOf("id_lodgify"),
      aiRTId = alHdr.indexOf("RoomTypeIds"), aiProp = alHdr.indexOf("Propiedad"),
      aiDpto = alHdr.indexOf("# Departamento");
  function _resolveAloj(hid, rtids) {
    for (var kk = 0; kk < alRows.length; kk++) {
      var hh = aiHId  >= 0 ? String(alRows[kk][aiHId]  || "").trim() : "";
      var il = aiIdLg >= 0 ? String(alRows[kk][aiIdLg] || "").trim() : "";
      var rt = aiRTId >= 0 ? String(alRows[kk][aiRTId] || "").trim() : "";
      var matches = false;
      if (hid && (hh === hid || il === hid)) matches = true;
      else if (rtids && rt === rtids) matches = true;
      if (matches) return {
        propiedad: aiProp >= 0 ? String(alRows[kk][aiProp] || "").trim() : "",
        depto:    aiDpto >= 0 ? String(alRows[kk][aiDpto] || "").trim() : ""
      };
    }
    return { propiedad:"", depto:"" };
  }
  var results = [];
  for (var i = 0; i < vals.length; i++) {
    var rowPh = String(vals[i][iPh] || "").replace(/\D/g,"");
    if (!rowPh || rowPh.length < 10 || rowPh.slice(-10) !== p10) continue;
    dbg.phone_matches++;
    var status = iSt >= 0 ? String(vals[i][iSt] || "").toLowerCase() : "";
    var da = iDA >= 0 ? String(vals[i][iDA] || "").trim() : "";
    var dd = iDD >= 0 ? String(vals[i][iDD] || "").trim() : "";
    ...
    var houseName = iHN >= 0 ? String(vals[i][iHN] || "").trim() : "";
    var code = iCC >= 0 ? String(vals[i][iCC] || "").trim() : "";
    if (!code && iCB >= 0) code = String(vals[i][iCB] || "").trim();
    results.push({
      Id: iId >= 0 ? vals[i][iId] : "",
      Source: iSrc >= 0 ? String(vals[i][iSrc] || "") : "",
      ConfirmationCode: code,
      GuestName: iGN >= 0 ? String(vals[i][iGN] || "") : "",
      GuestEmail: iGE >= 0 ? String(vals[i][iGE] || "") : "",
      HouseName: houseName || aloj.propiedad,
      Propiedad: aloj.propiedad || houseName,
      Departamento: aloj.depto,
      DateArrival: da,
      DateDeparture: dd,
      ...
      Registrado:     !!registradoMap[String(iId >= 0 ? vals[i][iId] : "").trim()],
    });
  }
...
function _toIsoDateForCompare_(s) {
...
function reservacionSetFolioByLodgifyId_(data) {
  ...
  // Columnas opcionales de montos que emit-auto también actualiza
  var amtCols = {
    "$ MONTO TOTAL Airbnb":       data && data.total_airbnb,
    "$ Comisión Airbnb":          data && data.comision_airbnb,
    "$ Monto antes de impuestos": data && data.monto_antes,
    "($) Monto Total pagado":     data && data.total_pagado,
    "$ Monto facturado Total":    data && data.monto_facturado,
  };
  var last = sh.getLastRow();
  var foundRow = -1;
  if (last >= 2) {
  ...
  function _writeAmts(row) {
    Object.keys(amtCols).forEach(function(k){
      var v = amtCols[k];
      if (v == null || v === "") return;
...
function perfilUpsertByPhone_(data) {
  var payload = (data && data.payload) || data || {};
  var phoneRaw = payload["Cel/Whatsapp (principal)"] || payload.phone || (data && data.phone);
  var p10 = _normalizePhone10_(phoneRaw);
  if (!p10) return { ok:false, error:"phone (o Cel/Whatsapp (principal)) requerido" };
  var sh = getSheet_(PERFILES_SHEET);
  var headers = getHeaders_(sh);
  var iPh = headers.indexOf("Cel/Whatsapp (principal)");
  if (iPh < 0) return { ok:false, error:"columna Cel/Whatsapp (principal) no existe en Perfiles" };
  ...
  if (last >= 2) {
    ...
    for (var i = 0; i < vals.length; i++) {
      var raw = String(vals[i][iPh] || "").replace(/\D/g,"");
      if (raw.length >= 10 && raw.slice(-10) === p10) { rowIdx = i + 2; break; }
    }
  }
  var now = new Date();
  var out = { ok:true, phone:p10, created:false, updated:false };
  if (rowIdx < 0) {
    // Crear fila nueva. ID_Perfil autoincremental si existe columna.
    var iId = headers.indexOf("ID_Perfil");
    var iFC = headers.indexOf("Fecha creación");
    var iFA = headers.indexOf("Fecha actualización");
    var row = new Array(headers.length).fill("");
    row[iPh] = "+" + p10;
    if (iId >= 0) row[iId] = "P" + Date.now() + "_" + Math.floor(Math.random()*1000);
    if (iFC >= 0) row[iFC] = now;
    if (iFA >= 0) row[iFA] = now;
    // Aplicar los campos del payload (excepto phone que ya se puso).
    Object.keys(payload).forEach(function(k){
      if (k === "phone") return;
      var ci = headers.indexOf(k);
      if (ci >= 0) row[ci] = payload[k];
    });
    sh.appendRow(row);
    out.created = true;
    ...
    // Update in-place.
    var iFA2 = headers.indexOf("Fecha actualización");
    Object.keys(payload).forEach(function(k){
      if (k === "phone") return;
      var ci = headers.indexOf(k);
      if (ci >= 0) sh.getRange(rowIdx, ci+1).setValue(payload[k]);
    });
    if (iFA2 >= 0) sh.getRange(rowIdx, iFA2+1).setValue(now);
    out.updated = true;
    out.row = rowIdx;
  }
  return out;
...
// ═══════════════════════════════════════════════════════════════════════════
// ║ Hoja Emergency_Contacts — números de emergencia persistidos.            ║
// ║ Reciben aviso cuando el bot crea reporte P1/crítico y son los que      ║
// ║ aparecen en el dropdown "Avisar" de cada card de Notificaciones + la   ║
// ║ ventana "Emergencia" del módulo Chats bot.                              ║
// ═══════════════════════════════════════════════════════════════════════════
var EMERGENCY_SHEET = "Emergency_Contacts";
var EMERGENCY_HEADERS = ["Nombre", "Telefono", "Activo", "Notas", "Updated_at", "Updated_by"];
function emergencyContactsList_() {
  // Fuente: hoja sys_users — columnas 'Nombre' + 'cel'. Todos los usuarios
  // con cel poblado se consideran contactos de emergencia (usan el sistema
  // por lo que son destinatarios naturales de avisos críticos).
  var ss = getSpreadsheet_();
  var sh = ss.getSheetByName('sys_users');
  if (!sh) return { ok:true, phones:[], contacts:[] };
  var last = sh.getLastRow();
  if (last < 2) return { ok:true, phones:[], contacts:[] };
  var headers = sh.getRange(1, 1, 1, sh.getLastColumn()).getValues()[0];
  var iNombre = -1, iCel = -1, iPuesto = -1;
  for (var h = 0; h < headers.length; h++) {
    var hn = String(headers[h] || "").trim().toLowerCase();
    if (hn === "nombre") iNombre = h;
    else if (hn === "cel") iCel = h;
    else if (hn === "puesto") iPuesto = h;
  }
  if (iNombre < 0 || iCel < 0) return { ok:false, error:"Faltan columnas 'Nombre' o 'cel' en sys_users" };
  var vals = sh.getRange(2, 1, last-1, sh.getLastColumn()).getValues();
  var contacts = [], phones = [];
  for (var i = 0; i < vals.length; i++) {
    var nombre = String(vals[i][iNombre] || "").trim();
    var rawCel = String(vals[i][iCel] || "").replace(/\D/g, "");
    if (!rawCel || rawCel.length < 10) continue;
    // Normaliza a +52NNNNNNNNNN (10 dígitos MX asumido por default).
    var tel = rawCel.length === 10 ? ("+52" + rawCel) : ("+" + rawCel);
    var puesto = iPuesto >= 0 ? String(vals[i][iPuesto] || "").trim() : "";
    contacts.push({ nombre: nombre, telefono: tel, puesto: puesto });
    phones.push(tel);
  }
  return { ok:true, phones: phones, contacts: contacts, source: "sys_users" };
}
function emergencyContactsSet_(data) {
  var payload = (data && data.payload) || data || {};
  var phones = Array.isArray(payload.phones) ? payload.phones : [];
  var contacts = Array.isArray(payload.contacts) ? payload.contacts : null;
  var sh = waEnsureSheet_(EMERGENCY_SHEET, EMERGENCY_HEADERS);
  var last = sh.getLastRow();
  var prev = {};
  if (last >= 2) {
    var oldVals = sh.getRange(2, 1, last-1, EMERGENCY_HEADERS.length).getValues();
    for (var i = 0; i < oldVals.length; i++) {
      var tel = String(oldVals[i][1] || "").trim();
      if (tel) prev[tel] = { nombre: oldVals[i][0], notas: oldVals[i][3] };
    }
    sh.getRange(2, 1, last-1, EMERGENCY_HEADERS.length).clearContent();
  }
  var now = new Date();
  var user = String(payload.updated_by || "");
  var rows = [];
  if (contacts && contacts.length) {
    for (var c = 0; c < contacts.length; c++) {
      var ct = contacts[c] || {};
      var telC = String(ct.telefono || "").trim();
      if (!telC) continue;
      var pC = prev[telC] || {};
      rows.push([String(ct.nombre || pC.nombre || "").trim(), telC, "true", String(ct.notas || pC.notas || "").trim(), now, user]);
    }
  } else {
    for (var j = 0; j < phones.length; j++) {
      var telJ = String(phones[j] || "").trim();
      if (!telJ) continue;
      var pJ = prev[telJ] || {};
      rows.push([String(pJ.nombre || ""), telJ, "true", String(pJ.notas || ""), now, user]);
    }
  }
  if (rows.length) sh.getRange(2, 1, rows.length, EMERGENCY_HEADERS.length).setValues(rows);
  return { ok:true, count: rows.length, phones: rows.map(function(r){ return r[1]; }) };
}

// ═══════════════════════════════════════════════════════════════════════════
// ║ Hoja Llaves_Notas — comentario libre por Propiedad, editable desde el   ║
// ║ módulo Llaves. Una fila por propiedad (case-insensitive).               ║
// ═══════════════════════════════════════════════════════════════════════════
var LLAVES_NOTAS_SHEET = "Llaves_Notas";
var LLAVES_NOTAS_HEADERS = ["Propiedad", "Notas", "Updated_at", "Updated_by"];
function _llavesNotasKey_(s) { return String(s || "").trim().toLowerCase(); }
function llavesNotasList_() {
  var sh = waEnsureSheet_(LLAVES_NOTAS_SHEET, LLAVES_NOTAS_HEADERS);
  var last = sh.getLastRow();
  if (last < 2) return { ok:true, notas: {} };
  var vals = sh.getRange(2, 1, last-1, LLAVES_NOTAS_HEADERS.length).getValues();
  var out = {};
  for (var i = 0; i < vals.length; i++) {
    var prop = String(vals[i][0] || "").trim();
    if (!prop) continue;
    out[_llavesNotasKey_(prop)] = {
      propiedad: prop,
      notas: String(vals[i][1] || ""),
      updated_at: vals[i][2] instanceof Date ? vals[i][2].toISOString() : String(vals[i][2] || ""),
      updated_by: String(vals[i][3] || ""),
    };
  }
  return { ok:true, notas: out };
}
function llavesNotasSet_(data) {
  var payload = (data && data.payload) || data || {};
  var propiedad = String(payload.propiedad || "").trim();
  if (!propiedad) return { ok:false, error:"propiedad requerida" };
  var notas = String(payload.notas || "");
  var user = String(payload.updated_by || "");
  var sh = waEnsureSheet_(LLAVES_NOTAS_SHEET, LLAVES_NOTAS_HEADERS);
  var last = sh.getLastRow();
  var rowIdx = -1;
  if (last >= 2) {
    ...
    for (var i = 0; i < vals.length; i++) {
      if (_llavesNotasKey_(vals[i][0]) === _llavesNotasKey_(propiedad)) { rowIdx = i + 2; break; }
    }
  }
  var now = new Date();
  if (rowIdx < 0) {
    sh.appendRow([propiedad, notas, now, user]);
  } else {
...
function asistenciaMarcar_(data) {
  ...
  if (missing.length) {
    var startCol = sh.getLastColumn() + 1;
    sh.getRange(1, startCol, 1, missing.length).setValues([missing]);
    hdr = sh.getRange(1, 1, 1, sh.getLastColumn()).getValues()[0].map(function(h){ return String(h||"").trim(); });
  }
  var iID = hdr.indexOf("ID");
  ...
  if (rowFound > 0) {
    ...
    // Formato homogéneo con el resto de la hoja: "yyyy-MM-dd HH:mm:ss"
  ...
  // Fuerza flush antes de leer para asegurar que la marca de hora recién
  // escrita está en la fila (setValue en Apps Script es async hasta flush).
  SpreadsheetApp.flush();
